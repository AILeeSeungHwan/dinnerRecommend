...
function LoadingOverlay() {
  ...
  useEffect(() => {
    const t1 = setInterval(() => setFrame(f => (f+1) % frames.length), 180)
    const t2 = setInterval(() => setMsgIdx(i => (i+1) % msgs.length), 1100)
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ fontSize:'3.5rem', marginBottom:16, lineHeight:1 }}>{frames[frame]}</div>
        <div style={{ fontSize:'1rem', fontWeight:700, color:'var(--text)', marginBottom:8 }}>
          AI가 고르는 중
        </div>
        <div style={{ fontSize:'.84rem', color:'var(--muted)', marginBottom:20 }}>{msgs[msgIdx]}</div>
        {/* 점 애니메이션 */}
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          fontSize:'5rem', lineHeight:1, marginBottom:16,
          transition:'all 0.1s',
          filter:`hue-rotate(${count*20}deg)`,
        }}>{face}</div>
...
function AiApp() {
  ...
  const [pendingRandom, setPendingRandom] = useState(null)
  ...
  function getRandom() {
    ...
    setPendingRandom(res)
    setDicing(true)
  }

  function onDiceFinish() {
    setDicing(false)
    setResults(pendingRandom)
  ...
  async function getRecommendations() {
    ...
    try {
      ...
      // ── 토큰 절감: top8만, 초압축 포맷 ──────────────────────
      const top8 = preScore(ctx, moods, weather, cands).slice(0, 8)
      const compact = top8.map(r =>
        `${r.name}|${r.type}|${r.rt}★|${r.priceRange||'?'}원|${(r.tags||[]).slice(0,3).join('·')}|${(r.moods||[]).join('·')}|${(r.rv||[]).slice(0,1).map(v=>v.replace(/ \(실제.*?\)/g,'')).join('')}`
      ).join('\n')

      const prompt = `삼성역맛집 TOP3 추천.
입력:"${ctx||'없음'}" 날씨:${weather||'무관'} 기분:${moods.join(',')||'무관'}${exit4Only?' 4번출구':''}
후보:\n${compact}
JSON만:{recommendations:[{rank:1,restaurantName:"이름",reason:"2문장추천이유",reviewHighlight:"리뷰한줄",matchScore:95},{rank:2,...},{rank:3,...}]}`
          ...
          <div style={{ display:'flex', flexWrap:'wrap', gap:5 }}>
            {MOODS.map(m=>(
              <button key={m} onClick={()=>setMoods(prev=>prev.includes(m)?prev.filter(x=>x!==m):[...prev,m])}
                style={btnStyle(moods.includes(m),'var(--accent)')}>{m}</button>
            ))}
          </div>
        </div>
        ...
        {results && (
          <div ref={resultsRef} style={{ marginTop:24 }}>
            ...
            {results.map((rec, i) => {
              ...
              return (
                <div key={i} style={{ background:'var(--surface2)', border:'1px solid var(--border)', borderLeft:`3px solid ${borderColors[i]}`, borderRadius:14, padding:'16px 14px', marginBottom:12 }}>
                  ...
                  <p style={{ fontSize:'.84rem', color:'#d0d0e0', marginBottom:8, lineHeight:1.65 }}>{rec.reason}</p>
                  {rec.reviewHighlight && (
