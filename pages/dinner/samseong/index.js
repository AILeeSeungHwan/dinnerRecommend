      ...
      if (needsExternal) {
        ...
        try {
          extRes = await fetch('/api/web-search-recommend', {
            ...
            body: JSON.stringify({ menuQuery: ctx.trim(), area: '삼성역', usageCount: getUsageCount() }),
            signal: wCtrl.signal,
          })
        } catch (wErr) {
          clearTimeout(wTimer)
        ...
        base = exit4Only ? restaurants.filter(r=>r.exit4) : restaurants
      }
...
const usageCnt = getUsageCount()
            const prompt = [
        '삼성역 맛집 큐레이터. 아래 후보들 중에서 요청에 맞는 곳 정확히 3곳 추천.',
