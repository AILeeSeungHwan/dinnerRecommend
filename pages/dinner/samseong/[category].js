...
export async function getStaticPaths() {
  return {
    paths: Object.keys(CATEGORY_MAP).map(slug => ({
...
export default function CategoryPage({ category, catInfo, restaurants }) {
  ...
  return (
    <>
      <Head>
        <title>삼성역 {catInfo.name} 맛집 추천 {restaurants.length}선 | dinner.ambitstock</title>
        ...
        <link rel="canonical" href={`https://dinner.ambitstock.com/dinner/samseong/${category}`} />
        <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
          "@context": "https://schema.org",
          "@type": "ItemList",
          "name": `삼성역 ${catInfo.name} 맛집`,
          "url": `https://dinner.ambitstock.com/dinner/samseong/${category}`,
      ...
      <div className="container" style={{ padding: '24px 16px' }}>
        ...
        {/* SEO 콘텐츠 블록 */}
        <article style={{
          marginTop: 48, padding: '28px 20px',
