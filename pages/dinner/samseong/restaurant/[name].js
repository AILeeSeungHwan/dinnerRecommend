  ...
  return (
    <Layout title={`${r.name} | 삼성역 ${r.type}`} description={metaDesc} canonical={pageUrl}>
      ...
      <article style={{ maxWidth:760, margin:'0 auto', padding:'28px 16px 60px' }}>
        ...
        <OperatorNote restaurant={r} regionName="삼성역" totalRegionRestaurants={831} govData={govData} />
        ...
        {matchedWx.length > 0 && (
          <>
        <PostMidCategoryBanner region="samseong" regionName="삼성역" restaurant={r} />
        ...
        {similar?.length > 0 && (
          <>
        <h2 style={h2style}>🍽️ 삼성역 {(r.cat && r.cat[0]) || r.type} 맛집 더 보기</h2>
            <p style={pstyle}>
              <strong>{r.name}</strong>와 같은 카테고리에서 평점·가격·거리 기준으로 한 곳씩 골라봤습니다.
            </p>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(180px, 1fr))', gap:12, marginBottom:28 }}>
              {similar.map((s, i) => (
                <SimilarRestaurantCard key={i} restaurant={s} regionPath="/dinner/samseong" />
              ))}
            </div>
          </>
        )}
