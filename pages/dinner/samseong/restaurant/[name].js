...
function buildIntro(r) {
  ...
  if (cats.includes('국밥') || cats.includes('국물')) {
    ...
    if (isHighRated) {
      return { emoji:'🍲', lines:[
        `삼성역에서 ${type}으로 ⭐${rt}점을 유지한다는 게 어떤 의미인지,`,
  ...
  if (cats.includes('일식') || tags.some(t => t.includes('오마카세') || t.includes('스시') || t.includes('초밥'))) {
    if (tags.some(t => t.includes('오마카세'))) {
      return { emoji:'🍣🌙', lines:[
        ...
        `⭐${rt}점. 삼성역에서 이 수준이라면 충분히 특별한 저녁입니다.`,
      ]}
    }
    return { emoji:'🍣', lines:[
      `신선한 네타 한 점, 샤리와의 황금 비율.`,
      `${name}에서 그 완성도를 느껴보세요.`,
      `⭐${rt}점, ${isCheap ? '부담 없는 가격까지 챙긴' : '삼성역 숨은'} 일식 맛집.`,
    ]}
  }
  ...
  if (cats.includes('양식') || cats.includes('이탈리안') || cats.includes('스테이크')) {
    if (moods.includes('데이트')) {
      return { emoji:'🍷', lines:[
        ...
        `${name}. 분위기·맛·가격 세 박자가 맞는 삼성역 ${type} 맛집.`,
...
export default function RestaurantPage({ restaurant: r, similar }) {
  ...
  const faqSchema = {
    ...
    "mainEntity": [
      ...
      { "@type":"Question", "name":`${r.name} 주차 가능한가요?`, "acceptedAnswer":{ "@type":"Answer", "text": r.parking ? '주차 가능합니다. 상세한 주차 정보는 매장에 문의하세요.' : '삼성역 인근 공영주차장 또는 코엑스 주차장을 이용하시거나 대중교통을 권장합니다.' } },
  ...
  const breadcrumbSchema = {
    ...
    "itemListElement": [
      ...
      slug && { "@type":"ListItem", "position":3, "name":`삼성역 ${catName}`, "item":`https://dinner.ambitstock.com/dinner/samseong/category/${slug}` },
      { "@type":"ListItem", "position": slug ? 4 : 3, "name":r.name, "item":pageUrl },
    ].filter(Boolean)
  }

  return (
    <Layout title={`${r.name} | 삼성역 ${r.type}`} description={metaDesc} canonical={pageUrl}>
      <PageTracker slug={`samseong-restaurant-${encodeURIComponent(r.name)}`} title={`${r.name} | 삼성역 ${r.type}`} />
      <Head>
        <meta name="keywords" content={`${r.name}, 삼성역 ${r.type}, 강남 ${r.type}, ${r.tags?.join(', ')}`} />
      ...
      <section style={{ background:'var(--surface)', borderBottom:'1px solid var(--border)', padding:'28px 16px' }}>
        <div style={{ maxWidth:760, margin:'0 auto' }}>
          ...
          <div style={{ display:'flex', gap:8, marginTop:16, flexWrap:'wrap' }}>
            ...
            <Link href="/dinner/samseong?tab=roulette"
      ...
      <article style={{ maxWidth:760, margin:'0 auto', padding:'28px 16px 60px' }}>
        ...
        {/* CTA 배너 — 지역 AI 추천 유도 */}
        <a href="/dinner/samseong?tab=roulette" style={{ textDecoration:'none', display:'block', marginBottom:20 }}>
          <div style={{
