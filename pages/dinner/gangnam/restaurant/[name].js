...
export async function getStaticProps({ params }) {
  ...
  return { props: { restaurant: { ...r, rv: r.rv || [], tags: r.tags || [], moods: r.moods || [], scene: r.scene || [], cat: r.cat || [], keywords: r.keywords || [], menuItems: r.menuItems || [], tel: r.tel || '', parking: r.parking || false, reservation: r.reservation || false, naverBlogCnt: r.naverBlogCnt || 0, naverPlaceId: r.naverPlaceId || '', naverUrl: r.naverUrl || '', imageUrl: r.imageUrl || '', imageUrl2: r.imageUrl2 || '', imageUrl3: r.imageUrl3 || '', imageUrl4: r.imageUrl4 || '' }, similar } }
}
...
function buildIntro(r) {
  ...
  if (cats.includes('일식') || tags.some(t => t.includes('오마카세') || t.includes('스시') || t.includes('초밥'))) {
    if (tags.some(t => t.includes('오마카세'))) {
      return { emoji:'🍣🌙', lines:[
        ...
        `⭐${rt}점. 강남역에서 이 수준이라면 충분히 특별한 저녁입니다.`,
      ]}
    }
    return { emoji:'🍣', lines:[
      `신선한 네타 한 점, 샤리와의 황금 비율.`,
      `${name}에서 그 완성도를 느껴보세요.`,
      `⭐${rt}점, ${isCheap ? '부담 없는 가격까지 챙긴' : '강남역 숨은'} 일식 맛집.`,
    ]}
  }
  ...
  if (cats.includes('양식') || cats.includes('이탈리안') || cats.includes('스테이크')) {
    if (moods.includes('데이트')) {
      return { emoji:'🍷', lines:[
        ...
        `${name}. 분위기·맛·가격 세 박자가 맞는 강남역 ${type} 맛집.`,
  ...
  if (cats.includes('치킨')) {
    return { emoji:'🍺🐔', lines:[
      ...
      `⭐${rt}점, ${cnt}명이 선택한 강남역 치킨 맛집.`,
    ]}
  }

  // ── fallback ──
  return { emoji: r.e || '🍽️', lines:[
    `강남역에서 ${type}을 찾는다면, 선택지를 좁혀드립니다.`,
...
export default function RestaurantPage({ restaurant: r, similar }) {
  ...
  const pageUrl = `https://dinner.ambitstock.com/dinner/gangnam/restaurant/${encodeURIComponent(r.name)}`
  ...
  // 메타 desc
  const metaDesc = `${r.name} — 강남역 ${r.type} 맛집. ${r.addr} 위치, 영업시간 ${formatHours(r.hours)}. 평점 ⭐${r.rt} (${r.cnt?.toLocaleString()}개 리뷰). ${r.tags?.slice(0,3).join('·')} 특징. 오늘뭐먹지 AI 추천.`
  ...
  const faqSchema = {
    ...
    "mainEntity": [
      ...
      { "@type":"Question", "name":`${r.name} 위치(주소)는?`, "acceptedAnswer":{ "@type":"Answer", "text":`서울 강남구 ${r.addr} (강남역 근처)` } },
      ...
      { "@type":"Question", "name":`${r.name} 주차 가능한가요?`, "acceptedAnswer":{ "@type":"Answer", "text": r.parking ? '주차 가능합니다. 상세한 주차 정보는 매장에 문의하세요.' : '강남역 인근 공영주차장 또는 코엑스 주차장을 이용하시거나 대중교통을 권장합니다.' } },
  ...
  const breadcrumbSchema = {
    ...
    "itemListElement": [
      ...
      slug && { "@type":"ListItem", "position":3, "name":`강남역 ${catName}`, "item":`https://dinner.ambitstock.com/dinner/gangnam/category/${slug}` },
      { "@type":"ListItem", "position": slug ? 4 : 3, "name":r.name, "item":pageUrl },
    ].filter(Boolean)
  }

  return (
    <Layout title={`${r.name} | 강남역 ${r.type}`} description={metaDesc} canonical={pageUrl}>
      <PageTracker slug={`gangnam-restaurant-${encodeURIComponent(r.name)}`} title={`${r.name} | 강남역 ${r.type}`} />
      <Head>
        <meta name="keywords" content={`${r.name}, 강남역 ${r.type}, 강남 ${r.type}, ${r.tags?.join(', ')}`} />
        <meta property="og:title" content={`${r.name} — 강남역 ${r.type} 맛집`} />
      ...
      <div style={{ background:'var(--surface)', borderBottom:'1px solid var(--border)', padding:'10px 16px' }}>
        <div style={{ maxWidth:760, margin:'0 auto', fontSize:'.75rem', color:'var(--muted)', display:'flex', gap:5, flexWrap:'wrap', alignItems:'center' }}>
          ...
          <Link href="/dinner/gangnam" style={{ color:'var(--muted)' }}>강남역 맛집</Link> <span>›</span>
          {slug && <><Link href={`/dinner/gangnam/category/${slug}`} style={{ color:'var(--muted)' }}>강남역 {catName}</Link> <span>›</span></>}
      ...
      <section style={{ background:'var(--surface)', borderBottom:'1px solid var(--border)', padding:'28px 16px' }}>
        <div style={{ maxWidth:760, margin:'0 auto' }}>
          ...
          <div style={{ display:'flex', gap:8, marginTop:16, flexWrap:'wrap' }}>
            ...
            <Link href="/dinner/gangnam"
              style={{ padding:'9px 18px', borderRadius:10, background:'var(--surface2)', border:'1px solid var(--border)', color:'var(--text)', fontSize:'.85rem', textDecoration:'none' }}>
              ✨ AI 맞춤 추천 받기
            </Link>
            <Link href="/dinner/gangnam?tab=roulette"
      ...
      <article style={{ maxWidth:760, margin:'0 auto', padding:'28px 16px 60px' }}>
        ...
        {/* CTA 배너 — 지역 AI 추천 유도 */}
        <a href="/dinner/gangnam?tab=roulette" style={{ textDecoration:'none', display:'block', marginBottom:20 }}>
          <div style={{
            ...
            <div style={{ flex:1 }}>
              <div style={{ color:'#fff', fontWeight:900, fontSize:'1rem', marginBottom:4, lineHeight:1.3 }}>
                강남역 뭐 먹을지 아직 못 정하셨나요?
              </div>
              <div style={{ color:'rgba(255,255,255,.85)', fontSize:'.8rem', marginBottom:10 }}>
                코엑스·테헤란로·강남 맛집 AI 추천 — 날씨·기분·예산에 맞게 AI가 바로 골라드려요
              </div>
              <span style={{
        ...
        <table style={{ width:'100%', borderCollapse:'collapse', fontSize:'.88rem', marginBottom:28 }}>
          <tbody>
            {[
              ...
              ['강남역 4번출구', r.exit4 ? '✅ 도보 3분 이내' : '강남역 도보권 내'],
        ...
        <p style={pstyle}>
          <strong>{r.name}</strong>은 서울 강남구 {r.addr}에 위치한 강남역 맛집입니다.
          {r.exit4
            ? ' 강남역 4번출구에서 도보 3분 이내로 접근성이 매우 좋습니다.'
            : ' 강남역에서 도보로 이동 가능합니다. 정확한 경로는 지도를 참고해주세요.'}
        </p>
        <ul style={ulstyle}>
          <li style={listyle}><strong>지하철</strong> — 2호선·수인분당선 강남역 하차</li>
          {r.exit4
            ? <li style={listyle}><strong>도보</strong> — 4번출구 기준 약 3분 이내</li>
            : <li style={listyle}><strong>도보</strong> — 강남역 각 출구에서 도보 5~10분 내외</li>}
        ...
        {[
          ...
          [`${r.name} 주소(위치)는 어디인가요?`, `서울특별시 강남구 ${r.addr}에 위치합니다. 강남역${r.exit4 ? ' 4번출구에서 도보 3분 거리' : ' 인근'}입니다.`],
        ...
        <div style={{ display:'flex', gap:10, flexWrap:'wrap', paddingTop:20, borderTop:'1px solid var(--border)' }}>
          {slug && (
            <Link href={`/dinner/gangnam/category/${slug}`}
