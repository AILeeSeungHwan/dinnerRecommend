            ...
            <>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(240px, 1fr))', gap:10 }}>
                {matchedRestaurants.map((r, i) => (
                  <Link href={`/dinner/gangnam/restaurant/${encodeURIComponent(r.name)}`} key={r.name + i} style={{ textDecoration:'none', color:'inherit' }}>
                    <div style={{
...
function naverMapUrl(name) {
  const cleaned = name
    .replace(/ (강남역점|강남역|강남점|삼성점|강남대로점|대치점|선릉점|강남점|삼성본점)$/, '')
    .replace(/ ([0-9]+호점)$/, '')
    .trim()
  return `https://map.naver.com/v5/search/${encodeURIComponent(cleaned + ' 강남역')}`
}
      ...
      <div style={{ background:'var(--surface)',border:'1px solid var(--border)',borderRadius:20,padding:'28px 24px',maxWidth:380,width:'100%',textAlign:'center',boxShadow:'0 24px 64px rgba(0,0,0,.8)' }}
        ...
        {done && pickedR && (
          <div style={{ display:'flex',gap:8,justifyContent:'center',flexWrap:'wrap' }}>
            ...
            <a href={`https://map.naver.com/v5/search/${encodeURIComponent(pickedR.name + ' 강남역')}`}
        ...
        {done && pickedRec?._external && (
          <div style={{ display:'flex',gap:8,justifyContent:'center',marginTop:8 }}>
            <a href={`https://map.naver.com/v5/search/${encodeURIComponent(pickedRec.restaurantName + ' 강남역')}`}
              target="_blank" rel="noopener noreferrer"
...
const usageCnt = getUsageCount()
        ...
        (skipDbCheckRef.current && lastNoDataMenuRef.current
          ? `강남역 맛집 추천. 사용자가 원래 "${lastNoDataMenuRef.current}"을 찾았으나 DB에 없어 연관 매장을 추천함. 각 reason에서 반드시 "${lastNoDataMenuRef.current}과 비슷한 이유" 또는 "${lastNoDataMenuRef.current} 대신 이 집을 추천하는 이유"를 자연스럽게 녹여줘. 단순 나열 금지.`
          : '강남역 맛집 추천. 오늘 무엇을 원하는지 맥락을 파악해서 후보 중 딱 맞는 3곳 골라줘. 단순 나열 금지—상황과 감정에 맞게 큐레이션. reason에 "검색자"라는 단어 절대 금지.'),
          ...
          <div ref={resultsRef} style={{ marginTop:24, maxWidth:'100%', overflowX:'hidden' }}
              ...
              return pr ? (
                <div style={{ marginBottom:14,padding:'14px 16px',background:'rgba(255,107,53,.1)',border:'2px solid var(--primary)',borderRadius:12 }}>
                  ...
                  <div style={{ display:'flex',gap:6,flexWrap:'wrap' }}>
                    {prR && (
                      <Link href={`/dinner/gangnam/restaurant/${encodeURIComponent(prR.name)}`}
                    ...
                    {prR && <a href={`https://map.naver.com/v5/search/${encodeURIComponent(prR.name + ' 강남역')}`}
                      target="_blank" rel="noopener noreferrer"
            ...
            {results[0]?._external && (
              <div style={{ marginBottom:14,padding:'10px 14px',background:'rgba(108,99,255,.07)',border:'1px solid rgba(108,99,255,.25)',borderRadius:10 }}>
                ...
                <div style={{ fontSize:'.72rem',color:'var(--muted)' }}>강남역 커뮤니티·지도 앱에서 실시간 검색한 결과예요. 방문 전 영업 여부를 확인하세요.</div>
              </div>
            )}
            ...
            <div style={{
              ...
              {results.map((rec,i)=>{
                ...
                // ── 외부 검색 결과 카드 ──
                if (rec._external) {
                  const naverUrl = `https://map.naver.com/v5/search/${encodeURIComponent(rec.restaurantName + ' 강남역')}`
                  // 3가지 형식 분기
                  const cardStyle = [
                    // 1번: 메뉴 중심 - 가격 뱃지 강조
                    ...
                    <a key={i} href={naverUrl} target="_blank" rel="noopener noreferrer"
                      ...
                      <div style={{ background:cardStyle.bg, border:`1px solid var(--border)`, borderLeft:`3px solid ${accentColor}`, borderRadius:14, padding:'16px 14px', cursor:'pointer', transition:'border-color .15s', height:'100%' }}
                        ...
                        <div style={{ display:'flex', gap:6, marginTop:8, alignItems:'center' }}>
                          <span style={{ fontSize:'.75rem', padding:'5px 12px', borderRadius:8, background:'var(--surface)', border:'1px solid var(--border)', color:'var(--muted)' }}>📍 {rec.address || '강남역 근처'}</span>
              ...
              if (rec._notInDB && !r) {
                // DB에 없는 식당 → 네이버지도 카드
                return (
                  <a key={i} href={`https://map.naver.com/v5/search/${encodeURIComponent(rec.restaurantName + ' 강남역')}`}
                    target="_blank" rel="noopener noreferrer"
      ...
      <div className="thumb-grid">
        {visible.map((r,i)=>(
          <React.Fragment key={i}>
            <SimilarRestaurantCard restaurant={{ ...r, reason: r.rt > 0 ? `평점 ${r.rt}점·리뷰 ${(r.cnt||0).toLocaleString()}건` : `리뷰 ${(r.cnt||0).toLocaleString()}건` }} regionPath="/dinner/gangnam" />
            {(i + 1) % 12 === 0 && (
...
export default function SamseongPage() {
  ...
  return (
    <Layout title="강남역 맛집 추천 2026 | 강남대로·테헤란로·역삼동 AI 가이드" description="강남역 맛집 AI 추천 가이드. 강남대로·테헤란로·역삼동·신논현·논현동 주변 국밥·이자카야·한우·중식·일식·양식 등 검증된 170개+ 식당을 날씨·기분·예산·카테고리에 맞춰 3초 만에 추천. 회식·데이트·혼밥·점심 상황별, 카카오맵·네이버 평점·리뷰·메뉴 가격 한눈에 비교." canonical="https://dinner.ambitstock.com/dinner/gangnam">
      <Head>
        <title>강남역 맛집 추천 2026 | 강남대로·테헤란로·역삼동 국밥·이자카야·한우·중식 AI 추천 | 오늘뭐먹지</title>
        <meta name="description" content="강남역 맛집 AI 추천 가이드. 강남대로·테헤란로·역삼동·신논현·논현동 주변 국밥·이자카야·한우·중식·일식·양식 등 검증된 170개+ 식당을 날씨·기분·예산·카테고리에 맞춰 3초 만에 추천. 회식·데이트·혼밥·점심 상황별, 카카오맵·네이버 평점·리뷰·메뉴 가격 한눈에 비교." />
        <link rel="canonical" href="https://dinner.ambitstock.com/dinner/gangnam" />
        <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
          "@context":"https://schema.org",
          "@graph": [
            {
              "@type":"ItemList","name":"강남역 맛집 추천",
              "description":`강남역·강남대로·테헤란로 주변 ${restaurants.length}개+ 맛집 AI 추천. 국밥·이자카야·한우·중식 등 카테고리별 탐색.`,
              "url":"https://dinner.ambitstock.com/dinner/gangnam","numberOfItems":restaurants.length,
              "itemListElement":topRated.slice(0,10).map((r,i)=>({ "@type":"ListItem","position":i+1,"name":r.name,"url":`https://dinner.ambitstock.com/dinner/gangnam/restaurant/${encodeURIComponent(r.name)}`,"description":`${r.type} | ⭐${r.rt}` }))
            },
            {
              "@type":"BreadcrumbList",
              "itemListElement":[
      ...
      <section style={{ background:'linear-gradient(135deg, var(--surface) 0%, var(--bg) 100%)',padding:'32px 16px 24px',borderBottom:'1px solid var(--border)' }}>
        <div style={{ maxWidth:900,margin:'0 auto' }}>
          <div style={{ fontSize:'.75rem',color:'var(--muted)',marginBottom:8 }}>
            <Link href="/" style={{ color:'var(--muted)' }}>오늘뭐먹지</Link> › 강남역
          </div>
          <h1 style={{ fontSize:'clamp(1.4rem,5vw,2.2rem)',fontWeight:900,marginBottom:8,lineHeight:1.2 }}>🏙️ 강남역 맛집</h1>
          <p style={{ color:'var(--muted)',fontSize:'.88rem',marginBottom:14 }}>
            강남대로·파르나스·테헤란로 <strong style={{ color:'var(--text)' }}>{restaurants.length}개+</strong> 식당 AI 추천
          </p>
      ...
      <div style={{ maxWidth:900,margin:'0 auto',padding:'16px 16px' }}>
              ...
              {CATS.map(cat=>{
                ...
                return (
                  <div key={cat.slug} style={{
                    ...
                    onMouseLeave={e=>{ e.currentTarget.style.borderColor='var(--border)'; e.currentTarget.style.boxShadow='none' }}
                  >
                    <Link href={`/dinner/gangnam/category/${cat.slug}`} style={{ display:'block', textDecoration:'none', color:'inherit', flex:1 }}>
              ...
              {posts.filter(p => p.region === 'gangnam').map(p => (
                <Link key={p.slug} href={`/posts/${p.slug}`} style={{ textDecoration:'none' }}>
                  <div style={{ display:'flex', flexDirection:'column', gap:6, background:'var(--surface)', border:'1px solid var(--border)', borderRadius:12, padding:8, cursor:'pointer', height:'100%' }}>
                    <PostThumbnail imageUrl={p.thumbnail} region="강남역" category={p.category} slug={p.slug} size="100%" />
        ...
        <section style={{ marginTop:48,padding:'28px 20px',background:'var(--surface)',borderRadius:14,border:'1px solid var(--border)' }}>
          <h2 style={{ fontSize:'1.05rem',fontWeight:800,marginBottom:16 }}>강남역 맛집 가이드 — 강남대로·테헤란로·봉은사 구역별 정리</h2>
          <p style={{ color:'var(--muted)',fontSize:'.88rem',lineHeight:1.8,marginBottom:12 }}>
            강남역 맛집은 강남대로몰·파르나스·그랜드인터컨티넨탈 방향, 테헤란로 이면도로, 봉은사로 세 구역으로 나뉩니다. 강남대로몰 내부와 아셈타워 주변은 외부 비즈니스 손님 접대와 데이트 자리에 활용되는 고급 레스토랑이 집중되어 있으며, 테헤란로 이면도로는 금융사·IT기업 직장인 점심 수요를 받는 가성비 식당들이 포진합니다.
          </p>
          <p style={{ color:'var(--muted)',fontSize:'.88rem',lineHeight:1.8,marginBottom:12 }}>
            봉은사로는 강남대로와 대치동 사이에 위치해 조용한 골목 분위기를 원하는 방문객이 선호합니다. 이자카야·스시 바·오마카세 레스토랑이 봉은사로 이면에 다수 위치하며, 강남대로 혼잡을 피하면서 수준 높은 식사를 즐기고 싶을 때 좋은 선택지입니다. 강남역 4번출구 방향 대치동 인근에는 학원가 특성에 맞는 저렴한 한식·분식 식당이 밀집해 있습니다.
          </p>
          <p style={{ color:'var(--muted)',fontSize:'.88rem',lineHeight:1.8,marginBottom:12 }}>
            점심 가성비를 기준으로 보면, 테헤란로 이면도로의 국밥·정식·칼국수 식당은 8천~1만 3천 원대에서 한 끼를 해결할 수 있습니다. 강남대로몰 지하 푸드코트는 다양한 메뉴를 빠르게 해결하기에 편리하며, 11시 40분 이전에 입장하면 점심 피크를 피할 수 있습니다. 혼밥을 원한다면 카운터석이 있는 국밥 전문점이나 일식 단품 식당이 적합합니다.
          </p>
          <p style={{ color:'var(--muted)',fontSize:'.88rem',lineHeight:1.8,marginBottom:12 }}>
            회식 장소를 찾고 있다면, 강남 이면도로의 대형 고기구이·이자카야 전문점이 가장 접근성이 좋습니다. 10~30인 단체 수용 가능한 식당도 강남대로 인근 이면도로에 다수 위치하며, 강남대로몰 주차 연계로 원거리 참석자도 편하게 이용할 수 있습니다. 룸 예약은 3~5일 전에 전화 확인하는 것이 안전합니다.
          </p>
          <p style={{ color:'var(--muted)',fontSize:'.88rem',lineHeight:1.8,marginBottom:12 }}>
          ...
          <p style={{ color:'var(--muted)',fontSize:'.88rem',lineHeight:1.8,marginBottom:16 }}>
            강남역 맛집은 카테고리별 가이드 페이지에서 더 자세히 확인할 수 있습니다. 고기 맛집은 <Link href="/dinner/gangnam/category/meat" style={{ color:'var(--primary)' }}>강남역 고기·한우</Link>, 회식 장소는 <Link href="/dinner/gangnam/category/group" style={{ color:'var(--primary)' }}>강남역 회식·단체</Link>, 오마카세·접대 자리는 <Link href="/dinner/gangnam/category/premium" style={{ color:'var(--primary)' }}>강남역 접대·파인다이닝</Link> 페이지를 확인하세요.
          </p>
          <p style={{ color:'var(--muted)',fontSize:'.88rem',lineHeight:1.8 }}>
            오늘 날씨·기분·예산을 입력하면 AI가 강남역 주변 맛집 중 지금 상황에 딱 맞는 3곳을 3초 만에 골라드립니다.
          </p>
        </section>
