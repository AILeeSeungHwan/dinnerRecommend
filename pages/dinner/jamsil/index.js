            ...
            <>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(240px, 1fr))', gap:10 }}>
                {matchedRestaurants.map((r, i) => (
                  <Link href={`/dinner/jamsil/restaurant/${encodeURIComponent(r.name)}`} key={r.name + i} style={{ textDecoration:'none', color:'inherit' }}>
                    <div style={{
      ...
      <div className="thumb-grid">
        {visible.map((r,i)=>(
          <React.Fragment key={i}>
            <SimilarRestaurantCard restaurant={{ ...r, reason: r.rt > 0 ? `평점 ${r.rt}점·리뷰 ${(r.cnt||0).toLocaleString()}건` : `리뷰 ${(r.cnt||0).toLocaleString()}건` }} regionPath="/dinner/jamsil" />
            {(i + 1) % 12 === 0 && (
...
export default function SamseongPage() {
  ...
  const [activeTab, setActiveTab] = useState(() => {
    if (typeof window !== 'undefined') return sessionStorage.getItem('jamsil-tab') || 'roulette'
    return 'roulette'
  })
  ...
  useEffect(() => {
    ...
    if (t && ['roulette','ai','browse','categories'].includes(t)) {
      setActiveTab(t)
      sessionStorage.setItem('jamsil-tab', t)
    }
  }, [router.query.tab])
  const topRated = [...restaurants].sort((a,b)=>b.rt-a.rt).slice(0,6)

  return (
    <Layout title="잠실 맛집 추천 2026 | 롯데월드·석촌호수·방이먹자골목 AI 가이드" description="잠실 맛집 AI 추천 가이드. 롯데월드·석촌호수·방이먹자골목·잠실새내·송파나루 주변 국밥·이자카야·한우·중식·일식·양식 등 검증된 150개+ 식당을 날씨·기분·예산·카테고리에 맞춰 3초 만에 추천. 회식·데이트·혼밥·점심 상황별, 카카오맵·네이버 평점·리뷰·메뉴 가격 한눈에 비교." canonical="https://dinner.ambitstock.com/dinner/jamsil">
      <Head>
        <title>잠실 맛집 추천 2026 | 롯데월드·석촌호수·방이먹자골목 국밥·이자카야·한우·중식 AI 추천 | 오늘뭐먹지</title>
        <meta name="description" content="잠실 맛집 AI 추천 가이드. 롯데월드·석촌호수·방이먹자골목·잠실새내·송파나루 주변 국밥·이자카야·한우·중식·일식·양식 등 검증된 150개+ 식당을 날씨·기분·예산·카테고리에 맞춰 3초 만에 추천. 회식·데이트·혼밥·점심 상황별, 카카오맵·네이버 평점·리뷰·메뉴 가격 한눈에 비교." />
        <link rel="canonical" href="https://dinner.ambitstock.com/dinner/jamsil" />
        <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
          "@context":"https://schema.org",
          "@graph": [
            {
              "@type":"ItemList","name":"잠실 맛집 추천",
              "description":`잠실역·방이동·석촌호수 주변 ${restaurants.length}개+ 맛집 AI 추천.`,
              "url":"https://dinner.ambitstock.com/dinner/jamsil","numberOfItems":restaurants.length,
              "itemListElement":topRated.slice(0,10).map((r,i)=>({ "@type":"ListItem","position":i+1,"name":r.name,"url":`https://dinner.ambitstock.com/dinner/jamsil/restaurant/${encodeURIComponent(r.name)}`,"description":`${r.type} | ⭐${r.rt}` }))
            },
            {
              "@type":"BreadcrumbList",
              "itemListElement":[
