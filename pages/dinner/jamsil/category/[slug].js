  ...
  return (
    <>
      ...
      <Head>
        ...
        <meta name="description" content={`잠실역·방이먹자골목·석촌호수·잠실새내·롯데월드 주변 ${catInfo.name} 맛집 ${restaurants.length}곳을 카카오맵·네이버 평점·리뷰·메뉴 가격 기준으로 정리. ${catInfo.keywords}. 회식·데이트·혼밥·점심 상황별 추천, 영업시간·주차·예약 정보까지 한눈에.`} />
      ...
      <div className="container" style={{ padding:'24px 16px' }}>
        ...
        <div className="thumb-grid">
          {sorted.slice(0, visibleCount).map((r, i) => (
            <SimilarRestaurantCard key={i} restaurant={{ ...r, reason: r.rt > 0 ? `평점 ${r.rt}점·리뷰 ${(r.cnt||0).toLocaleString()}건` : `리뷰 ${(r.cnt||0).toLocaleString()}건` }} regionPath="/dinner/jamsil" />
          ))}
        </div>
