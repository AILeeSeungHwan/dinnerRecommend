...
export async function getStaticProps({ params }) {
  ...
  const govData = await fetchGovData('jamsil', r.name)
...
export default function RestaurantPage({ restaurant: r, similar, govData }) {
  ...
  const pageUrl = `https://dinner.ambitstock.com/dinner/jamsil/restaurant/${encodeURIComponent(r.name)}`
  ...
  // 메타 desc
  const metaDesc = `${r.name} — 잠실 ${r.type} 맛집. ${r.addr} 위치, 영업시간 ${formatHours(r.hours)}. 카카오맵 평점 ⭐${r.rt} (${r.cnt?.toLocaleString()}개 리뷰). ${r.tags?.slice(0,3).join('·')} 특징. 오늘뭐먹지 AI 추천.`
  ...
  const faqSchema = {
    ...
    "mainEntity": [
      ...
      { "@type":"Question", "name":`${r.name} 주차 가능한가요?`, "acceptedAnswer":{ "@type":"Answer", "text": r.parking ? '주차 가능합니다. 상세한 주차 정보는 매장에 문의하세요.' : '잠실역 인근 공영주차장 또는 롯데월드몰 주차장을 이용하시거나 대중교통을 권장합니다.' } },
  ...
  const breadcrumbSchema = {
    ...
    "itemListElement": [
      ...
      slug && { "@type":"ListItem", "position":3, "name":`잠실 ${catName}`, "item":`https://dinner.ambitstock.com/dinner/jamsil/category/${slug}` },
    ...
    <Layout title={`${r.name} | 잠실 ${r.type}`} description={metaDesc} canonical={pageUrl}>
      ...
      <div style={{ background:'var(--surface)', borderBottom:'1px solid var(--border)', padding:'10px 16px' }}>
        <div style={{ maxWidth:760, margin:'0 auto', fontSize:'.75rem', color:'var(--muted)', display:'flex', gap:5, flexWrap:'wrap', alignItems:'center' }}>
          ...
          {slug && <><Link href={`/dinner/jamsil/category/${slug}`} style={{ color:'var(--muted)' }}>잠실 {catName}</Link> <span>›</span></>}
        ...
        <div style={{ maxWidth:760, margin:'0 auto' }}>
          <div style={{ display:'flex', gap:14, alignItems:'flex-start' }}>
            ...
            <div style={{ flex:1, minWidth:0 }}>
              ...
              {r.addr && r.addr !== 'South Korea' && <p style={{ fontSize:'.84rem', color:'var(--muted)', marginBottom:4 }}>📍 서울 송파구 {r.addr}</p>}
          ...
          <div style={{ display:'flex', gap:8, marginTop:16, flexWrap:'wrap' }}>
            ...
            <Link href="/dinner/jamsil?tab=roulette"
      ...
      <article style={{ maxWidth:760, margin:'0 auto', padding:'28px 16px 60px' }}>
        ...
        <table style={{ width:'100%', borderCollapse:'collapse', fontSize:'.88rem', marginBottom:28 }}>
          <tbody>
            {[
              ['식당 종류', r.type],
              ['주소', r.addr && r.addr !== 'South Korea' ? `서울 송파구 ${r.addr}` : '위치 정보 준비 중'],
              ...
              ['잠실 4번출구', r.exit4 ? '✅ 도보 3분 이내' : '잠실 도보권 내'],
            ].map(([label, val], i) => (
        ...
        <OperatorNote restaurant={r} regionName="잠실" totalRegionRestaurants={1095} govData={govData} />
        ...
        <p style={pstyle}>
          ...
          {r.exit4
            ? ' 잠실 4번출구에서 도보 3분 이내로 접근성이 매우 좋습니다.'
            : ' 잠실에서 도보로 이동 가능합니다. 정확한 경로는 지도를 참고해주세요.'}
        </p>
        <ul style={ulstyle}>
          <li style={listyle}><strong>지하철</strong> — 2호선·수인분당선 잠실 하차</li>
          {r.exit4
            ? <li style={listyle}><strong>도보</strong> — 4번출구 기준 약 3분 이내</li>
            : <li style={listyle}><strong>도보</strong> — 잠실 각 출구에서 도보 5~10분 내외</li>}
        ...
        {similar?.length > 0 && (
          <>
        <h2 style={h2style}>🍽️ 잠실 {(r.cat && r.cat[0]) || r.type} 맛집 더 보기</h2>
            <p style={pstyle}>
              <strong>{r.name}</strong>와 같은 카테고리에서 평점·가격·거리 기준으로 한 곳씩 골라봤습니다.
            </p>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(180px, 1fr))', gap:12, marginBottom:28 }}>
              {similar.map((s, i) => (
                <SimilarRestaurantCard key={i} restaurant={s} regionPath="/dinner/jamsil" />
              ))}
            </div>
          </>
        )}
        ...
        <div style={{ display:'flex', gap:10, flexWrap:'wrap', paddingTop:20, borderTop:'1px solid var(--border)' }}>
          {slug && (
            <Link href={`/dinner/jamsil/category/${slug}`}
