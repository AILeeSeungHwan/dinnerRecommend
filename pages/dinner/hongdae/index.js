...
const ROULETTE_MENUS = [
  ...
  { emoji:'🥩', label:'삼겹살·고기구이', pattern:/삼겹살|돼지고기|고기구이|소금구이|제주흑돼지|돼지구이|^고기$/ },
  ...
  { emoji:'🍣', label:'초밥·회',       pattern:/초밥|롤|회|횟집|생선회|사시미|해산물|일식|스시오마카세/ },
  { emoji:'🍝', label:'파스타·피자',   pattern:/파스타|피자|이탈리|양식|스테이크|프렌치|프랑스/ },
  { emoji:'🥟', label:'짜장·짬뽕·중식', pattern:/중식|짜장|짬뽕|딤섬/ },
  { emoji:'🍗', label:'치킨',          pattern:/치킨|닭강정|닭갈비|닭볶음|닭구이|닭요리|패스트푸드/ },
...
function MenuRouletteTab() {
  ...
  function spin() {
    ...
    timerRef.current = setTimeout(() => {
      setSpinning(false)
      const menu = shuffledMenus[picked]
      setResult(menu)
    ...
    <div style={{ padding:'20px 16px' }}>
      ...
      <div style={{ display:'flex', flexDirection:'column', alignItems:'center', marginBottom:20, position:'relative' }}>
        ...
        <div style={{
          ...
          filter:'drop-shadow(0 2px 6px rgba(255,107,53,.5))',
        }} />
        {/* 회전하는 휠 */}
        <div ref={wheelRef} style={{ width:SIZE, height:SIZE }}>
          <svg viewBox={`0 0 ${SIZE} ${SIZE}`} width="100%" height="100%"
            style={{ borderRadius:'50%', boxShadow:'0 6px 32px rgba(0,0,0,.35)', display:'block', maxWidth:SIZE }}>
            {renderSlices()}
            {/* 중앙 버튼 */}
            <circle cx={CENTER} cy={CENTER} r={INNER_R} fill="var(--surface)" stroke="rgba(255,255,255,.4)" strokeWidth="3"
              style={{ filter:'drop-shadow(0 2px 8px rgba(0,0,0,.3))' }} />
            <text x={CENTER} y={CENTER - 6} textAnchor="middle" dominantBaseline="central"
              style={{ fontSize:'11px', fontWeight:900, fill:'var(--primary)' }}>SPIN</text>
            <text x={CENTER} y={CENTER + 8} textAnchor="middle" dominantBaseline="central"
              style={{ fontSize:'8px', fontWeight:600, fill:'var(--muted)' }}>▼</text>
          </svg>
        </div>
      </div>
      ...
      <div style={{ textAlign:'center', marginBottom:24 }}>
        <button onClick={()=>{; spin() }} disabled={spinning}
          style={{
            ...
            boxShadow: spinning ? 'none' : '0 4px 20px rgba(255,107,53,.3)',
            transition:'all .2s',
          }}>
      ...
      {result && !spinning && (
        <div style={{ marginTop:8 }}>
          <div style={{
            ...
            background:'linear-gradient(135deg, rgba(255,107,53,.08), rgba(108,99,255,.08))',
            border:'1px solid var(--primary)',
          }}>
            <div style={{ fontSize:'2rem' }}>{result.emoji}</div>
            <div style={{ fontSize:'1.15rem', fontWeight:900, color:'var(--primary)', marginTop:4 }}>
              오늘은 {result.label}!
            </div>
            {matchedRestaurants.length === 0 && (
              <div style={{ fontSize:'.82rem', color:'var(--muted)', marginTop:8 }}>해당 메뉴 식당이 없습니다.</div>
            )}
          </div>
            ...
            <>
              ...
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:12 }}>
                <div style={{ fontSize:'.88rem', fontWeight:700, color:'var(--text)' }}>
                  🍽️ 추천 매장 {matchedRestaurants.length}곳
                </div>
                <button onClick={reshuffleRestaurants}
                  style={{
                    padding:'6px 14px', borderRadius:10,
                    background:'var(--surface2)', border:'1px solid var(--border)',
                    color:'var(--accent)', fontSize:'.78rem', fontWeight:700, cursor:'pointer',
                  }}>
                  🔀 다른 매장 보기
                </button>
              </div>
              <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(240px, 1fr))', gap:10 }}>
                {matchedRestaurants.map((r, i) => (
                  <Link href={`/dinner/hongdae/restaurant/${encodeURIComponent(r.name)}`} key={r.name + i} style={{ textDecoration:'none', color:'inherit' }}>
                    <div style={{
                      background:'var(--surface)', border:'1px solid var(--border)', borderRadius:14,
                      padding:'16px', transition:'border-color .15s, box-shadow .15s', cursor:'pointer',
                    }}
                    ...
                    >
                      <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:8 }}>
                        ...
                        {r.exit4 && <span style={{ fontSize:'.6rem', color:'var(--accent)' }}>🚇</span>}
                      </div>
                      ...
                      {r.tags && r.tags.length > 0 && (
                        <div style={{ display:'flex', flexWrap:'wrap', gap:4, marginTop:6 }}>
                          {r.tags.slice(0,3).map(t => (
                            <span key={t} style={{ fontSize:'.65rem', padding:'1px 6px', borderRadius:100, background:'rgba(255,107,53,.08)', color:'var(--primary)' }}>#{t}</span>
                          ))}
                        </div>
                      )}
                    </div>
  ...
  const templates = [
    ...
    () => {
      ...
      const base = cnt >= 100
        ...
        : cnt > 0 ? `${cnt}명이 다녀갔다.` : `${r.type || '식당'}.`
      const reason = `${base}${tagStr ? `  ${tagStr}.` : ''}`
      return { reason, highlight: tagStr || base }
    },
    ...
    () => {
      ...
      const tagStr = tags.slice(0, 2).join(' · ')
      const reason = tagStr ? `${intro}  ${tagStr}` : intro
      return { reason, highlight: tagStr || sc }
    },

    // 3: 해시태그 나열
    () => {
      const tagStr = tags.slice(0, 4).map(t => `#${t}`).join('  ')
      const reason = tagStr || `${r.type || '맛집'}.`
      return { reason, highlight: `#${tags[0] || r.type}` }
    },
    ...
    () => {
      ...
      const tagStr = tags.slice(0, 2).join(' · ')
      const reason = badge ? `${badge}.${tagStr ? `  ${tagStr}` : ''}` : `${tagStr || r.type || '맛집'}.`
      return { reason, highlight: star || cntStr || tagStr }
    },
    ...
    () => {
      ...
      const tagStr = tags.slice(0, 2).join(' · ')
      const reason = `${when} 추천하는 ${r.type || '식당'}.${tagStr ? `  ${tagStr}` : ''}`
      return { reason, highlight: when }
    },

    // 6: 대표 태그 한 개 강조
    () => {
      if (!tags[0]) return templates[3]()
      const t0 = tags[0]
      ...
      const reason = extra ? `${intro}  ${extra}` : intro
      return { reason, highlight: t0 }
    },

    // 7: 태그 + 방문자 수
    () => {
      const tagLine = tags[0] ? `#${tags.slice(0,3).join(' #')}.` : ''
      const cntStr = cnt > 0 ? `방문객 ${cnt.toLocaleString()}명.` : ''
      const reason = [tagLine, cntStr].filter(Boolean).join('  ') || `${r.type || ''} 한 곳.`
      return { reason, highlight: tags[0] ? `#${tags[0]}` : cntStr }
    },
    ...
    () => {
      ...
      const cntStr = cnt >= 100 ? `${cnt.toLocaleString()}명 방문` : cnt > 0 ? `${cnt}명 방문` : ''
      const badge = [star, cntStr].filter(Boolean).join(' · ')
      const reason = badge ? `${badge}.` : `${r.type || '식당'}.`
      return { reason, highlight: star || cntStr }
    },
    ...
    () => {
      ...
      const reason = tagStr && price ? `${intro}  ${tagStr}` : intro
      return { reason, highlight: price || tagStr }
    },
    ...
    () => {
      ...
      const extra = tags.slice(1, 3).map(t => `#${t}`).join('  ')
      const reason = extra ? `${intro}  ${extra}` : intro
      return { reason, highlight: tagWord || repeat }
    },
    ...
    () => {
      ...
      const reason = `${intro}${tagLine ? '  '+tagLine : ''}`
      return { reason, highlight: tags[0] || sc }
    },
    ...
    () => {
      ...
      const star = rt ? `⭐${rt}` : ''
      const cntStr = cnt > 0 ? `${cnt.toLocaleString()}명` : ''
      const badge = [star, cntStr].filter(Boolean).join(' · ')
      const reason = [tagStr, badge].filter(Boolean).join('  ') || `${r.type || '맛집'}.`
      return { reason, highlight: tagStr || star }
    },
    ...
    () => {
      ...
      const intro = when
        ...
        : `${typeWord} 고민될 때 이 집.`
      const reason = tagStr ? `${intro}  ${tagStr}` : intro
      return { reason, highlight: typeWord || when }
    },
    ...
    () => {
      ...
      const intro = cnt >= 100
        ...
        : `${r.type || '맛집'} 추천.`
      const reason = `${intro} 대표 메뉴: ${menuStr}.`
    ...
    () => {
      ...
      const reason = `평균 ${avgStr}. ${menuNames}.`
    ...
    () => {
      ...
      const reason = [cntStr, menuStr ? `대표 메뉴는 ${menuStr}.` : ''].filter(Boolean).join(' ') || `${r.type || '맛집'}.`
      return { reason, highlight: menuStr || cntStr || '' }
    },
  ]
...
function naverMapUrl(name) {
  const cleaned = name
    .replace(/ (홍대입구점|홍대입구|홍대점|삼성점|홍대대로점|대치점|선릉점|홍대점|삼성본점)$/, '')
    .replace(/ ([0-9]+호점)$/, '')
    .trim()
  return `https://map.naver.com/v5/search/${encodeURIComponent(cleaned + ' 홍대입구')}`
}
      ...
      <div style={{ background:'var(--surface)',border:'1px solid var(--border)',borderRadius:20,padding:'28px 24px',maxWidth:380,width:'100%',textAlign:'center',boxShadow:'0 24px 64px rgba(0,0,0,.8)' }}
        ...
        {done && pickedR && (
          <div style={{ display:'flex',gap:8,justifyContent:'center',flexWrap:'wrap' }}>
            ...
            <a href={`https://map.naver.com/v5/search/${encodeURIComponent(pickedR.name + ' 홍대입구')}`}
        ...
        {done && pickedRec?._external && (
          <div style={{ display:'flex',gap:8,justifyContent:'center',marginTop:8 }}>
            <a href={`https://map.naver.com/v5/search/${encodeURIComponent(pickedRec.restaurantName + ' 홍대입구')}`}
              target="_blank" rel="noopener noreferrer"
...
const usageCnt = getUsageCount()
        ...
        (skipDbCheckRef.current && lastNoDataMenuRef.current
          ? `홍대입구 맛집 추천. 사용자가 원래 "${lastNoDataMenuRef.current}"을 찾았으나 DB에 없어 연관 매장을 추천함. 각 reason에서 반드시 "${lastNoDataMenuRef.current}과 비슷한 이유" 또는 "${lastNoDataMenuRef.current} 대신 이 집을 추천하는 이유"를 자연스럽게 녹여줘. 단순 나열 금지.`
          : '홍대입구 맛집 추천. 오늘 무엇을 원하는지 맥락을 파악해서 후보 중 딱 맞는 3곳 골라줘. 단순 나열 금지—상황과 감정에 맞게 큐레이션. reason에 "검색자"라는 단어 절대 금지.'),
          ...
          <div ref={resultsRef} style={{ marginTop:24, maxWidth:'100%', overflowX:'hidden' }}
              ...
              return pr ? (
                <div style={{ marginBottom:14,padding:'14px 16px',background:'rgba(255,107,53,.1)',border:'2px solid var(--primary)',borderRadius:12 }}>
                  ...
                  <div style={{ display:'flex',gap:6,flexWrap:'wrap' }}>
                    {prR && (
                      <Link href={`/dinner/hongdae/restaurant/${encodeURIComponent(prR.name)}`}
                    ...
                    {prR && <a href={`https://map.naver.com/v5/search/${encodeURIComponent(prR.name + ' 홍대입구')}`}
                      target="_blank" rel="noopener noreferrer"
            ...
            {results[0]?._external && (
              <div style={{ marginBottom:14,padding:'10px 14px',background:'rgba(108,99,255,.07)',border:'1px solid rgba(108,99,255,.25)',borderRadius:10 }}>
                ...
                <div style={{ fontSize:'.72rem',color:'var(--muted)' }}>홍대입구 커뮤니티·지도 앱에서 실시간 검색한 결과예요. 방문 전 영업 여부를 확인하세요.</div>
              </div>
            )}
            ...
            <div style={{
              ...
              {results.map((rec,i)=>{
                ...
                // ── 외부 검색 결과 카드 ──
                if (rec._external) {
                  const naverUrl = `https://map.naver.com/v5/search/${encodeURIComponent(rec.restaurantName + ' 홍대입구')}`
                  // 3가지 형식 분기
                  const cardStyle = [
                    // 1번: 메뉴 중심 - 가격 뱃지 강조
                    ...
                    <a key={i} href={naverUrl} target="_blank" rel="noopener noreferrer"
                      ...
                      <div style={{ background:cardStyle.bg, border:`1px solid var(--border)`, borderLeft:`3px solid ${accentColor}`, borderRadius:14, padding:'16px 14px', cursor:'pointer', transition:'border-color .15s', height:'100%' }}
                        ...
                        <div style={{ display:'flex', gap:6, marginTop:8, alignItems:'center' }}>
                          <span style={{ fontSize:'.75rem', padding:'5px 12px', borderRadius:8, background:'var(--surface)', border:'1px solid var(--border)', color:'var(--muted)' }}>📍 {rec.address || '홍대입구 근처'}</span>
              ...
              if (rec._notInDB && !r) {
                // DB에 없는 식당 → 네이버지도 카드
                return (
                  <a key={i} href={`https://map.naver.com/v5/search/${encodeURIComponent(rec.restaurantName + ' 홍대입구')}`}
                    target="_blank" rel="noopener noreferrer"
      ...
      <div className="thumb-grid">
        {visible.map((r,i)=>(
          <React.Fragment key={i}>
            <SimilarRestaurantCard restaurant={{ ...r, reason: r.rt > 0 ? `평점 ${r.rt}점·리뷰 ${(r.cnt||0).toLocaleString()}건` : `리뷰 ${(r.cnt||0).toLocaleString()}건` }} regionPath="/dinner/hongdae" />
            {(i + 1) % 12 === 0 && (
...
export default function SamseongPage() {
  ...
  const [activeTab, setActiveTab] = useState(() => {
    if (typeof window !== 'undefined') return sessionStorage.getItem('gangnam-tab') || 'roulette'
    return 'roulette'
  })
  const [pendingCat, setPendingCat] = useState(null)
  const switchTab = (tab) => {; setActiveTab(tab); sessionStorage.setItem('gangnam-tab', tab) }
  useEffect(() => {
    const t = router.query.tab
    if (t && ['roulette','ai','browse','categories'].includes(t)) {
      setActiveTab(t)
      sessionStorage.setItem('gangnam-tab', t)
    }
  }, [router.query.tab])
  const topRated = [...restaurants].sort((a,b)=>b.rt-a.rt).slice(0,6)

  return (
    <Layout title="홍대입구 맛집 추천 2026 | 홍대거리·연남동·합정 AI 가이드" description="홍대입구 맛집 AI 추천 가이드. 홍대거리·연남동·합정·상수동·서교동 주변 국밥·이자카야·한우·중식·일식·양식 등 검증된 170개+ 식당을 날씨·기분·예산·카테고리에 맞춰 3초 만에 추천. 회식·데이트·혼밥·점심 상황별, 카카오맵·네이버 평점·리뷰·메뉴 가격 한눈에 비교." canonical="https://dinner.ambitstock.com/dinner/hongdae">
      <Head>
        <title>홍대입구 맛집 추천 2026 | 홍대거리·연남동·합정·상수 국밥·이자카야·한우·중식 AI 추천 | 오늘뭐먹지</title>
        <meta name="description" content="홍대입구 맛집 AI 추천 가이드. 홍대거리·연남동·합정·상수동·서교동 주변 국밥·이자카야·한우·중식·일식·양식 등 검증된 170개+ 식당을 날씨·기분·예산·카테고리에 맞춰 3초 만에 추천. 회식·데이트·혼밥·점심 상황별, 카카오맵·네이버 평점·리뷰·메뉴 가격 한눈에 비교." />
        <link rel="canonical" href="https://dinner.ambitstock.com/dinner/hongdae" />
        <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
          "@context":"https://schema.org",
          "@graph": [
            {
              "@type":"ItemList","name":"홍대입구 맛집 추천",
              "description":`홍대입구·홍대대로·테헤란로 주변 ${restaurants.length}개+ 맛집 AI 추천. 국밥·이자카야·한우·중식 등 카테고리별 탐색.`,
              "url":"https://dinner.ambitstock.com/dinner/hongdae","numberOfItems":restaurants.length,
              "itemListElement":topRated.slice(0,10).map((r,i)=>({ "@type":"ListItem","position":i+1,"name":r.name,"url":`https://dinner.ambitstock.com/dinner/hongdae/restaurant/${encodeURIComponent(r.name)}`,"description":`${r.type} | ⭐${r.rt}` }))
            },
            {
              "@type":"BreadcrumbList",
              "itemListElement":[
      ...
      <section style={{ background:'linear-gradient(135deg, var(--surface) 0%, var(--bg) 100%)',padding:'32px 16px 24px',borderBottom:'1px solid var(--border)' }}>
        <div style={{ maxWidth:900,margin:'0 auto' }}>
          <div style={{ fontSize:'.75rem',color:'var(--muted)',marginBottom:8 }}>
            <Link href="/" style={{ color:'var(--muted)' }}>오늘뭐먹지</Link> › 홍대입구
          </div>
          <h1 style={{ fontSize:'clamp(1.4rem,5vw,2.2rem)',fontWeight:900,marginBottom:8,lineHeight:1.2 }}>🏙️ 홍대입구 맛집</h1>
          <p style={{ color:'var(--muted)',fontSize:'.88rem',marginBottom:14 }}>
            홍대대로·파르나스·테헤란로 <strong style={{ color:'var(--text)' }}>{restaurants.length}개+</strong> 식당 AI 추천
          </p>
      ...
      <div style={{ maxWidth:900,margin:'0 auto',padding:'16px 16px' }}>
        ...
        {activeTab==='categories' && (
          <div>
            ...
            <div style={{
              ...
              {CATS.map(cat=>{
                const count = cat.exit4Only
                  ? restaurants.filter(r=>r.exit4).length
                  : restaurants.filter(r=>
                      cat.cats.some(c=>r.cat?.includes(c)) ||
                      (cat.tags||[]).some(t=>r.tags?.includes(t))
                    ).length
                return (
                  <div key={cat.slug} style={{
                    ...
                    onMouseLeave={e=>{ e.currentTarget.style.borderColor='var(--border)'; e.currentTarget.style.boxShadow='none' }}
                  >
                    <Link href={`/dinner/hongdae/category/${cat.slug}`} style={{ display:'block', textDecoration:'none', color:'inherit', flex:1 }}>
              ...
              {posts.filter(p => p.region === 'hongdae').map(p => (
                <Link key={p.slug} href={`/posts/${p.slug}`} style={{ textDecoration:'none' }}>
                  <div style={{ display:'flex', flexDirection:'column', gap:6, background:'var(--surface)', border:'1px solid var(--border)', borderRadius:12, padding:8, cursor:'pointer', height:'100%' }}>
                    <PostThumbnail imageUrl={p.thumbnail} region="홍대입구" category={p.category} slug={p.slug} size="100%" />
        ...
        <section style={{ marginTop:48,padding:'28px 20px',background:'var(--surface)',borderRadius:14,border:'1px solid var(--border)' }}>
          <h2 style={{ fontSize:'1.05rem',fontWeight:800,marginBottom:16 }}>홍대입구 맛집 가이드 — 홍대대로·테헤란로·봉은사 구역별 정리</h2>
          <p style={{ color:'var(--muted)',fontSize:'.88rem',lineHeight:1.8,marginBottom:12 }}>
            홍대입구 맛집은 홍대대로몰·파르나스·그랜드인터컨티넨탈 방향, 테헤란로 이면도로, 봉은사로 세 구역으로 나뉩니다. 홍대대로몰 내부와 아셈타워 주변은 외부 비즈니스 손님 접대와 데이트 자리에 활용되는 고급 레스토랑이 집중되어 있으며, 테헤란로 이면도로는 금융사·IT기업 직장인 점심 수요를 받는 가성비 식당들이 포진합니다.
          </p>
          <p style={{ color:'var(--muted)',fontSize:'.88rem',lineHeight:1.8,marginBottom:12 }}>
            봉은사로는 홍대대로와 대치동 사이에 위치해 조용한 골목 분위기를 원하는 방문객이 선호합니다. 이자카야·스시 바·오마카세 레스토랑이 봉은사로 이면에 다수 위치하며, 홍대대로 혼잡을 피하면서 수준 높은 식사를 즐기고 싶을 때 좋은 선택지입니다. 홍대입구역 9번출구 방향 대치동 인근에는 학원가 특성에 맞는 저렴한 한식·분식 식당이 밀집해 있습니다.
          </p>
          <p style={{ color:'var(--muted)',fontSize:'.88rem',lineHeight:1.8,marginBottom:12 }}>
            점심 가성비를 기준으로 보면, 테헤란로 이면도로의 국밥·정식·칼국수 식당은 8천~1만 3천 원대에서 한 끼를 해결할 수 있습니다. 홍대대로몰 지하 푸드코트는 다양한 메뉴를 빠르게 해결하기에 편리하며, 11시 40분 이전에 입장하면 점심 피크를 피할 수 있습니다. 혼밥을 원한다면 카운터석이 있는 국밥 전문점이나 일식 단품 식당이 적합합니다.
          </p>
          <p style={{ color:'var(--muted)',fontSize:'.88rem',lineHeight:1.8,marginBottom:12 }}>
            회식 장소를 찾고 있다면, 홍대 이면도로의 대형 고기구이·이자카야 전문점이 가장 접근성이 좋습니다. 10~30인 단체 수용 가능한 식당도 홍대대로 인근 이면도로에 다수 위치하며, 홍대대로몰 주차 연계로 원거리 참석자도 편하게 이용할 수 있습니다. 룸 예약은 3~5일 전에 전화 확인하는 것이 안전합니다.
          </p>
          <p style={{ color:'var(--muted)',fontSize:'.88rem',lineHeight:1.8,marginBottom:12 }}>
            접대·파인다이닝은 파르나스호텔 인근 예약제 오마카세와 코스 레스토랑이 중심입니다. 홍대 비즈니스 상권의 핵심 지역인 만큼 개인실을 갖춘 접대 레스토랑이 다른 지역 대비 많으며, 예약 경쟁도 치열합니다. 1인 코스 기준 10만~30만 원 이상 범위로, 최소 1주일 전 예약이 권장됩니다.
          </p>
          <p style={{ color:'var(--muted)',fontSize:'.88rem',lineHeight:1.8,marginBottom:16 }}>
            홍대입구 맛집은 카테고리별 가이드 페이지에서 더 자세히 확인할 수 있습니다. 고기 맛집은 <Link href="/dinner/hongdae/category/meat" style={{ color:'var(--primary)' }}>홍대입구 고기·한우</Link>, 회식 장소는 <Link href="/dinner/hongdae/category/group" style={{ color:'var(--primary)' }}>홍대입구 회식·단체</Link>, 오마카세·접대 자리는 <Link href="/dinner/hongdae/category/premium" style={{ color:'var(--primary)' }}>홍대입구 접대·파인다이닝</Link> 페이지를 확인하세요.
          </p>
          <p style={{ color:'var(--muted)',fontSize:'.88rem',lineHeight:1.8 }}>
            오늘 날씨·기분·예산을 입력하면 AI가 홍대입구 주변 맛집 중 지금 상황에 딱 맞는 3곳을 3초 만에 골라드립니다.
          </p>
        </section>
