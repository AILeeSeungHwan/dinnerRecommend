...
export default function RestaurantPage({ restaurant: r, similar }) {
  ...
  const faqSchema = {
    ...
    "mainEntity": [
      ...
      { "@type":"Question", "name":`${r.name} 위치(주소)는?`, "acceptedAnswer":{ "@type":"Answer", "text":`서울 강남구 ${r.addr} (영통 근처)` } },
  ...
  return (
    <Layout title={`${r.name} | 영통 ${r.type}`} description={metaDesc} canonical={pageUrl}>
      ...
      <article style={{ maxWidth:760, margin:'0 auto', padding:'28px 16px 60px' }}>
        ...
        <p style={pstyle}>
          <strong>{r.name}</strong>은 서울 강남구 {r.addr}에 위치한 영통 맛집입니다.
          {r.exit4
        ...
        {[
          ...
          [`${r.name} 주소(위치)는 어디인가요?`, `서울특별시 강남구 ${r.addr}에 위치합니다. 영통${r.exit4 ? ' 4번출구에서 도보 3분 거리' : ' 인근'}입니다.`],
