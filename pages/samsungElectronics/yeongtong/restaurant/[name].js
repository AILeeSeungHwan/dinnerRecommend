...
export default function RestaurantPage({ restaurant: r, similar, govData }) {
  ...
  // 메타 desc
  const metaDesc = `${r.name} — 영통 ${r.type} 맛집. ${r.addr} 위치, 영업시간 ${formatHours(r.hours)}. 카카오맵 평점 ⭐${r.rt} (${r.cnt?.toLocaleString()}개 리뷰). ${r.tags?.slice(0,3).join('·')} 특징. 오늘뭐먹지 AI 추천.`
  ...
  const faqSchema = {
    ...
    "mainEntity": [
      ...
      { "@type":"Question", "name":`${r.name} 위치(주소)는?`, "acceptedAnswer":{ "@type":"Answer", "text":`수원 영통구 ${r.addr} (영통역 근처)` } },
      ...
      { "@type":"Question", "name":`${r.name} 주차 가능한가요?`, "acceptedAnswer":{ "@type":"Answer", "text": r.parking ? '주차 가능합니다. 상세한 주차 정보는 매장에 문의하세요.' : '영통역 인근 공영주차장을 이용하시거나 대중교통을 권장합니다.' } },
  ...
  const breadcrumbSchema = {
    ...
    "itemListElement": [
      ...
      { "@type":"ListItem", "position":2, "name":"영통 맛집", "item":"https://dinner.ambitstock.com/samsungElectronics/yeongtong" },
      slug && { "@type":"ListItem", "position":3, "name":`영통 ${catName}`, "item":`https://dinner.ambitstock.com/samsungElectronics/yeongtong/category/${slug}` },
  ...
  return (
    <Layout title={`${r.name} | 영통 ${r.type}`} description={metaDesc} canonical={pageUrl}>
      ...
      <section style={{ background:'var(--surface)', borderBottom:'1px solid var(--border)', padding:'28px 16px' }}>
        <div style={{ maxWidth:760, margin:'0 auto' }}>
          ...
          <div style={{ display:'flex', gap:8, marginTop:16, flexWrap:'wrap' }}>
            ...
            <Link href="/samsungElectronics/yeongtong?tab=roulette"
      ...
      <article style={{ maxWidth:760, margin:'0 auto', padding:'28px 16px 60px' }}>
        ...
        {/* CTA 배너 — 지역 AI 추천 유도 */}
        <a href="/samsungElectronics/yeongtong?tab=roulette" style={{ textDecoration:'none', display:'block', marginBottom:20 }}>
          <div style={{
        ...
        <OperatorNote restaurant={r} regionName="영통" totalRegionRestaurants={498} govData={govData} />
        ...
        {similar?.length > 0 && (
          <>
        <h2 style={h2style}>🍽️ 영통 {(r.cat && r.cat[0]) || r.type} 맛집 더 보기</h2>
            <p style={pstyle}>
              <strong>{r.name}</strong>와 같은 카테고리에서 평점·가격·거리 기준으로 한 곳씩 골라봤습니다.
            </p>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(180px, 1fr))', gap:12, marginBottom:28 }}>
              {similar.map((s, i) => (
                <SimilarRestaurantCard key={i} restaurant={s} regionPath="/samsungElectronics/yeongtong" />
              ))}
            </div>
          </>
        )}
