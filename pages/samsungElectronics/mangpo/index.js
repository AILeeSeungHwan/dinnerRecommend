...
function AiApp({ pendingCat, onPendingCatUsed }) {
  ...
  async function getRecommendations() {
    ...
    try {
      ...
      const prompt = `당신은 망포역·수원 삼성전자 맛집 전문가입니다. 아래 조건에 맞는 식당 3곳을 후보 목록에서 골라 추천해주세요.
...
export default function JamsilPage() {
  ...
  return (
    <Layout
      title="망포역 맛집 AI 추천"
      description="망포역·영통지구 맛집 AI 추천. 고기구이·국밥·이자카야·로컬 맛집."
      canonical="https://dinner.ambitstock.com/samsungElectronics/mangpo"
    >
      <Head>
        <title>망포역 맛집 추천 | 삼성전자 망포 AI 추천 | 뭐먹지</title>
        <meta name="description" content={`망포역·영통지구 맛집 AI 추천. 고기구이·국밥·이자카야·한식 ${restaurants.length}개+ 식당.`} />
        <link rel="canonical" href="https://dinner.ambitstock.com/samsungElectronics/mangpo" />
        <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
          "@context":"https://schema.org","@type":"ItemList","name":"망포역 맛집 추천",
      ...
      <section style={{ background:'linear-gradient(135deg, var(--surface) 0%, var(--bg) 100%)',padding:'32px 16px 24px',borderBottom:'1px solid var(--border)' }}>
        <div style={{ maxWidth:900,margin:'0 auto' }}>
          <div style={{ fontSize:'.75rem',color:'var(--muted)',marginBottom:8 }}>
            <Link href="/" style={{ color:'var(--muted)' }}>오늘뭐먹지</Link> › 망포역
          </div>
          <h1 style={{ fontSize:'clamp(1.4rem,5vw,2.2rem)',fontWeight:900,marginBottom:8,lineHeight:1.2 }}>🚇 망포역 맛집</h1>
          <p style={{ color:'var(--muted)',fontSize:'.88rem',marginBottom:14 }}>
            망포역·영통지구·삼성전자 인근 <strong style={{ color:'var(--text)' }}>{restaurants.length}개+</strong> 식당 AI 추천
          </p>
          <div style={{ display:'flex',flexWrap:'wrap',gap:6 }}>
            {['#회식', '#직장인점심', '#고기구이', '#로컬맛집', '#망포맛집'].map(t=>(
      ...
      <div style={{ maxWidth:900,margin:'0 auto',padding:'20px 16px' }}>
            ...
            {CATS.map(cat=>{
              ...
              return (
                <div key={cat.slug} style={{ position:'relative' }}>
                  <Link href={`/samsungElectronics/mangpo/category/${cat.slug}`}>
        ...
        <div style={{ marginBottom:36, paddingTop:8 }}>
          ...
          <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fill, minmax(120px, 1fr))',gap:8 }}>
            {CATS.map(cat=>{
              ...
              return (
                <div key={cat.slug} style={{ position:'relative' }}>
                  <Link href={`/samsungElectronics/mangpo/category/${cat.slug}`} style={{ textDecoration:'none' }}>
                    <div
        ...
        <article style={{ marginTop:48,padding:'24px 20px',background:'var(--surface)',borderRadius:14,border:'1px solid var(--border)' }}>
          <h2 style={{ fontSize:'1rem',fontWeight:800,marginBottom:12 }}>망포역 맛집 가이드</h2>
          <p style={{ color:'var(--muted)',fontSize:'.88rem',lineHeight:1.8,marginBottom:10 }}>
            망포역 맛집은 <strong style={{ color:'var(--text)' }}>영통지구 주거 상권</strong>과 삼성전자 인근 직장인 수요가 함께 형성된 지역입니다. 로컬 맛집 비율이 높고 가성비가 우수합니다.
          </p>
          <p style={{ color:'var(--muted)',fontSize:'.88rem',lineHeight:1.8,marginBottom:10 }}>
            <strong style={{ color:'var(--text)' }}>망포역 주변</strong>에는 삼겹살·갈비 등 고기구이 전문점과 국밥·찌개류 한식당이 많아 직장인 점심과 소규모 회식에 적합합니다.
          </p>
          <p style={{ color:'var(--muted)',fontSize:'.88rem',lineHeight:1.8 }}>
            <strong style={{ color:'var(--text)' }}>영통지구 주거단지 상권</strong>에는 저녁 시간대 이자카야·포차 스타일 식당과 족발·보쌈 전문점이 활성화되어 있습니다. 동네 단골 맛집 찾기에 최적입니다.
          </p>
        </article>
