  ...
  return (
    <Layout title={`${r.name} | 영통구청 ${r.type}`} description={metaDesc} canonical={pageUrl}>
      ...
      <article style={{ maxWidth:760, margin:'0 auto', padding:'28px 16px 60px' }}>
        ...
        {matchedWx.length > 0 && (
          <>
        <CategorySuggestButton restaurant={r} region="yeongtongGu" />
        ...
        {similar?.length > 0 && (
          <>
        <h2 style={h2style}>🍽️ 영통구청 {r.type} 맛집 더 보기</h2>
            <p style={pstyle}>
              <strong>{r.name}</strong>와 비슷한 영통구청 {r.type} 맛집을 더 추천해드립니다.
            </p>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(200px, 1fr))', gap:10, marginBottom:28 }}>
              {similar.map((s, i) => (
                <Link href={`/samsungElectronics/yeongtongGu/restaurant/${encodeURIComponent(s.name)}`} key={i}>
