...
export async function getStaticProps({ params }) {
  ...
  const govData = await fetchGovData('yeongtongGu', r.name)
...
function buildIntro(r) {
  ...
  if (cats.includes('일식') || tags.some(t => t.includes('오마카세') || t.includes('스시') || t.includes('초밥'))) {
    if (tags.some(t => t.includes('오마카세'))) {
      return { emoji:'🍣🌙', lines:[
        ...
        `⭐${rt}점. 영통구청에서 이 수준이라면 충분히 특별한 저녁입니다.`,
      ]}
    }
    return { emoji:'🍣', lines:[
      `신선한 네타 한 점, 샤리와의 황금 비율.`,
      `${name}에서 그 완성도를 느껴보세요.`,
      `⭐${rt}점, ${isCheap ? '부담 없는 가격까지 챙긴' : '영통구청 숨은'} 일식 맛집.`,
    ]}
  }
  ...
  if (cats.includes('양식') || cats.includes('이탈리안') || cats.includes('스테이크')) {
    if (moods.includes('데이트')) {
      return { emoji:'🍷', lines:[
        ...
        `${name}. 분위기·맛·가격 세 박자가 맞는 영통구청 ${type} 맛집.`,
  ...
  if (cats.includes('치킨')) {
    return { emoji:'🍺🐔', lines:[
      ...
      `⭐${rt}점, ${cnt}명이 선택한 영통구청 치킨 맛집.`,
    ]}
  }

  // ── fallback ──
  return { emoji: r.e || '🍽️', lines:[
    `영통구청에서 ${type}을 찾는다면, 선택지를 좁혀드립니다.`,
...
export default function RestaurantPage({ restaurant: r, similar, govData }) {
  ...
  const pageUrl = `https://dinner.ambitstock.com/samsungElectronics/yeongtongGu/restaurant/${encodeURIComponent(r.name)}`
  ...
  const faqSchema = {
    ...
    "mainEntity": [
      ...
      { "@type":"Question", "name":`${r.name} 위치(주소)는?`, "acceptedAnswer":{ "@type":"Answer", "text":`수원 영통구 ${r.addr} (영통구청 근처)` } },
      ...
      { "@type":"Question", "name":`${r.name} 주차 가능한가요?`, "acceptedAnswer":{ "@type":"Answer", "text": r.parking ? '주차 가능합니다. 상세한 주차 정보는 매장에 문의하세요.' : '영통구청 인근 공영주차장을 이용하시거나 대중교통을 권장합니다.' } },
  ...
  const breadcrumbSchema = {
    ...
    "itemListElement": [
      ...
      { "@type":"ListItem", "position":2, "name":"영통구청 맛집", "item":"https://dinner.ambitstock.com/samsungElectronics/yeongtongGu" },
      slug && { "@type":"ListItem", "position":3, "name":`영통구청 ${catName}`, "item":`https://dinner.ambitstock.com/samsungElectronics/yeongtongGu/category/${slug}` },
      { "@type":"ListItem", "position": slug ? 4 : 3, "name":r.name, "item":pageUrl },
    ].filter(Boolean)
  }

  return (
    <Layout title={`${r.name} | 영통구청 ${r.type}`} description={metaDesc} canonical={pageUrl}>
      <Head>
        <meta name="keywords" content={`${r.name}, 영통구청 ${r.type}, 강남 ${r.type}, ${r.tags?.join(', ')}`} />
        <meta property="og:title" content={`${r.name} — 영통구청 ${r.type} 맛집`} />
      ...
      <div style={{ background:'var(--surface)', borderBottom:'1px solid var(--border)', padding:'10px 16px' }}>
        <div style={{ maxWidth:760, margin:'0 auto', fontSize:'.75rem', color:'var(--muted)', display:'flex', gap:5, flexWrap:'wrap', alignItems:'center' }}>
          ...
          <Link href="/samsungElectronics/yeongtongGu" style={{ color:'var(--muted)' }}>영통구청 맛집</Link> <span>›</span>
          {slug && <><Link href={`/samsungElectronics/yeongtongGu/category/${slug}`} style={{ color:'var(--muted)' }}>영통구청 {catName}</Link> <span>›</span></>}
      ...
      <article style={{ maxWidth:760, margin:'0 auto', padding:'28px 16px 60px' }}>
        ...
        {/* CTA 배너 — 지역 AI 추천 유도 */}
        <a href="/samsungElectronics/yeongtongGu?tab=roulette" style={{ textDecoration:'none', display:'block', marginBottom:20 }}>
          <div style={{
        ...
        <OperatorNote restaurant={r} regionName="영통구청" totalRegionRestaurants={448} govData={govData} />
        ...
        {matchedWx.length > 0 && (
          <>
        <PostMidCategoryBanner region="yeongtongGu" regionName="영통구청" restaurant={r} />
        ...
        <p style={pstyle}>
          <strong>{r.name}</strong>은 수원 영통구 {r.addr}에 위치한 영통구청 맛집입니다.
          {r.exit4
            ? ' 영통구청 4번출구에서 도보 3분 이내로 접근성이 매우 좋습니다.'
            : ' 영통구청에서 도보로 이동 가능합니다. 정확한 경로는 지도를 참고해주세요.'}
        </p>
        <ul style={ulstyle}>
          <li style={listyle}><strong>지하철</strong> — 2호선·수인분당선 영통구청 하차</li>
          {r.exit4
            ? <li style={listyle}><strong>도보</strong> — 4번출구 기준 약 3분 이내</li>
            : <li style={listyle}><strong>도보</strong> — 영통구청 각 출구에서 도보 5~10분 내외</li>}
        ...
        <FaqAccordion defaultOpenAll items={[
          ...
          [`${r.name} 주소(위치)는 어디인가요?`, `경기도 수원시 영통구 ${r.addr}에 위치합니다. 영통구청구청${r.exit4 ? ' 인근' : ' 인근'}입니다.`],
        ...
        {similar?.length > 0 && (
          <>
        <h2 style={h2style}>🍽️ 영통구청 {(r.cat && r.cat[0]) || r.type} 맛집 더 보기</h2>
            <p style={pstyle}>
              <strong>{r.name}</strong>와 같은 카테고리에서 평점·가격·거리 기준으로 한 곳씩 골라봤습니다.
            </p>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(180px, 1fr))', gap:12, marginBottom:28 }}>
              {similar.map((s, i) => (
                <SimilarRestaurantCard key={i} restaurant={s} regionPath="/samsungElectronics/yeongtongGu" />
              ))}
            </div>
          </>
        )}
        ...
        <div style={{ display:'flex', gap:10, flexWrap:'wrap', paddingTop:20, borderTop:'1px solid var(--border)' }}>
          {slug && (
            <Link href={`/samsungElectronics/yeongtongGu/category/${slug}`}
