...
function naverMapUrl(name) {
  const cleaned = name
    ...
    .replace(/ ([0-9]+호점)$/, '')
    .trim()
  // 식당명에 영통구청 지역이 포함되면 그대로, 아니면 " 영통구청" 추가
  const hasRegion = /(영통구청|영통구|수원|영통)/.test(name)
...
export default function RestaurantPage({ restaurant: r, similar }) {
  ...
  const pageUrl = `${BASE}/samsungElectronics/yeongtongGu/restaurant/${encodeURIComponent(r.name)}`
  ...
  const faqSchema = {
    ...
    "mainEntity": [
      ...
      { "@type":"Question", "name":`${r.name} 위치(주소)는?`, "acceptedAnswer":{ "@type":"Answer", "text":`경기도 수원시 영통구 ${r.addr} (영통구청 근처)` } },
  ...
  const breadcrumbSchema = {
    ...
    "itemListElement": [
      ...
      { "@type":"ListItem", "position":2, "name":"영통구청 맛집", "item":`${BASE}/samsungElectronics/yeongtongGu` },
  ...
  return (
    <Layout title={`${r.name} | 영통구청 ${r.type}`} description={metaDesc} canonical={pageUrl}>
      <Head>
        <meta name="keywords" content={`${r.name}, 영통구청 ${r.type}, 영통구청 맛집, 수원 영통 맛집, ${r.tags?.join(', ')}`} />
      ...
      <article style={{ maxWidth:760, margin:'0 auto', padding:'28px 16px 60px' }}>
        ...
        <p style={ps}>
          <strong>{r.name}</strong>은 경기도 수원시 {r.addr}에 위치한 영통구청 맛집입니다.
 영통구청역에서 도보로 이동 가능합니다. 정확한 경로는 지도를 참고해주세요.
        </p>
        <ul style={uls}>
          <li style={lis}><strong>지하철</strong> — 수인분당선 영통구청역 하차</li>
          <li style={lis}><strong>도보</strong> — 영통구청 인근 각 정류장에서 도보 5~10분 내외</li>
          <li style={lis}><strong>주차</strong> — {r.parking ? '주차 가능 (매장 문의)' : '영통구청 인근 공영주차장 이용 권장'}</li>
        </ul>
        <a href={mapUrl} target="_blank" rel="noopener noreferrer"
        ...
        {[
          ...
          [`${r.name} 주소(위치)는 어디인가요?`, `경기도 수원시 영통구 ${r.addr}에 위치합니다. 영통구청 근처입니다.`],
        ...
        {similar?.length > 0 && (
          <>
            <h2 style={h2s}>🍽️ 영통구청 {r.type} 맛집 더 보기</h2>
            <p style={ps}><strong>{r.name}</strong>와 비슷한 영통구청 {r.type} 맛집을 더 추천해드립니다.</p>
