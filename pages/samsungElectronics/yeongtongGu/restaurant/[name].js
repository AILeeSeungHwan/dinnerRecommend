...
export default function RestaurantPage({ restaurant: r, similar }) {
  ...
  const faqSchema = {
    ...
    "mainEntity": [
      ...
      { "@type":"Question", "name":`${r.name} 위치(주소)는?`, "acceptedAnswer":{ "@type":"Answer", "text":`서울 강남구 ${r.addr} (영통구청 근처)` } },
      ...
      { "@type":"Question", "name":`${r.name} 주차 가능한가요?`, "acceptedAnswer":{ "@type":"Answer", "text":'영통구청 인근 공영주차장 또는 코엑스 주차장을 이용하시거나 대중교통을 권장합니다.' } },
    ]
  }
  ...
  const breadcrumbSchema = {
    ...
    "itemListElement": [
      ...
      { "@type":"ListItem", "position":2, "name":"영통구청 맛집", "item":"https://gangnamwhat.com/samsungElectronics/yeongtongGu" },
      slug && { "@type":"ListItem", "position":3, "name":`영통구청 ${catName}`, "item":`https://gangnamwhat.com/samsungElectronics/yeongtongGu/category/${slug}` },
  ...
  return (
    <Layout title={`${r.name} | 영통구청 ${r.type}`} description={metaDesc} canonical={pageUrl}>
      ...
      <article style={{ maxWidth:760, margin:'0 auto', padding:'28px 16px 60px' }}>
        ...
        <p style={pstyle}>
          <strong>{r.name}</strong>은 서울 강남구 {r.addr}에 위치한 영통구청 맛집입니다.
          {r.exit4
        ...
        {[
          ...
          [`${r.name} 주소(위치)는 어디인가요?`, `서울특별시 강남구 ${r.addr}에 위치합니다. 영통구청${r.exit4 ? ' 4번출구에서 도보 3분 거리' : ' 인근'}입니다.`],
