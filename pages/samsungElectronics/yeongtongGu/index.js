            ...
            <>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(240px, 1fr))', gap:10 }}>
                {matchedRestaurants.map((r, i) => (
                  <Link href={`/samsungElectronics/yeongtongGu/restaurant/${encodeURIComponent(r.name)}`} key={r.name + i} style={{ textDecoration:'none', color:'inherit' }}>
...
const usageCnt = getUsageCount()
        ...
        (skipDbCheckRef.current && lastNoDataMenuRef.current
          ? `영통구청 맛집 추천. 사용자가 원래 "${lastNoDataMenuRef.current}"을 찾았으나 DB에 없어 연관 매장을 추천함. 각 reason에서 반드시 "${lastNoDataMenuRef.current}과 비슷한 이유" 또는 "${lastNoDataMenuRef.current} 대신 이 집을 추천하는 이유"를 자연스럽게 녹여줘. 단순 나열 금지.`
          : '영통구청 맛집 추천. 오늘 무엇을 원하는지 맥락을 파악해서 후보 중 딱 맞는 3곳 골라줘. 단순 나열 금지—상황과 감정에 맞게 큐레이션. reason에 "검색자"라는 단어 절대 금지.'),
          ...
          <div ref={resultsRef} style={{ marginTop:24, maxWidth:'100%', overflowX:'hidden' }}
              ...
              return pr ? (
                <div style={{ marginBottom:14,padding:'14px 16px',background:'rgba(255,107,53,.1)',border:'2px solid var(--primary)',borderRadius:12 }}>
                  ...
                  <div style={{ display:'flex',gap:6,flexWrap:'wrap' }}>
                    {prR && (
                      <Link href={`/samsungElectronics/yeongtongGu/restaurant/${encodeURIComponent(prR.name)}`}
            ...
            <div style={{
              ...
              if (rec._notInDB && !r) {
                // DB에 없는 식당 → 네이버지도 카드
                return (
                  <a key={i} href={`https://map.naver.com/v5/search/${encodeURIComponent(rec.restaurantName + ' 영통구청')}`}
                    target="_blank" rel="noopener noreferrer"
      ...
      <div className="thumb-grid">
        {visible.map((r,i)=>(
          <React.Fragment key={i}>
            <SimilarRestaurantCard restaurant={{ ...r, reason: r.rt > 0 ? `평점 ${r.rt}점·리뷰 ${(r.cnt||0).toLocaleString()}건` : `리뷰 ${(r.cnt||0).toLocaleString()}건` }} regionPath="/samsungElectronics/yeongtongGu" />
            {(i + 1) % 12 === 0 && (
...
export default function SamseongPage() {
  ...
  const [activeTab, setActiveTab] = useState(() => {
    if (typeof window !== 'undefined') return sessionStorage.getItem('yeongtongGu-tab') || 'roulette'
    return 'roulette'
  })
  ...
  useEffect(() => {
    ...
    if (t && ['roulette','ai','browse','categories'].includes(t)) {
      setActiveTab(t)
      sessionStorage.setItem('yeongtongGu-tab', t)
    }
  }, [router.query.tab])
              ...
              {posts.filter(p => p.region === 'yeongtongGu').map(p => (
                <Link key={p.slug} href={`/posts/${p.slug}`} style={{ textDecoration:'none' }}>
                  <div style={{ display:'flex', flexDirection:'column', gap:6, background:'var(--surface)', border:'1px solid var(--border)', borderRadius:12, padding:8, cursor:'pointer', height:'100%' }}>
                    <PostThumbnail imageUrl={p.thumbnail} region="영통구청" category={p.category} slug={p.slug} size="100%" />
