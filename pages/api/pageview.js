 ...
 *   HEALTH_SUPABASE_URL         = https://xxxxxx.supabase.co
 *   HEALTH_SUPABASE_SERVICE_KEY = eyJ... (service_role key)
 */
...
function detectDevice(ua) {
  ...
  const bots = ['bot', 'spider', 'crawl', 'fetch', 'lighthouse', 'pagespeed', 'semrush', 'ahrefs', 'yandex', 'baidu', 'google-read-aloud']
...
export default async function handler(req, res) {
  ...
  if (!supabase) return res.json({ ok: false, reason: 'HEALTH_SUPABASE_URL / HEALTH_SUPABASE_SERVICE_KEY not configured' })
  ...
  const sessionHash = createHash('sha256').update(ip + ua + kstDate).digest('hex').slice(0, 16)
