...
export default async function handler(req, res) {
  ...
  const fromDate =
    from ||
    new Date(Date.now() + 9 * 3600000 - 7 * 86400000)
      .toISOString()
      .slice(0, 10);
  const toDate =
    to || new Date(Date.now() + 9 * 3600000).toISOString().slice(0, 10);
  ...
  rows.forEach((r) => {
    ...
    if (!dateMap[date])
      dateMap[date] = { date, total: 0, search: 0, by_source: {} };
    dateMap[date].total++;
  ...
  const by_date = Object.values(dateMap).sort((a, b) =>
    a.date.localeCompare(b.date),
  );

  const recent = rows.slice(0, 100).map((r) => ({
    time: new Date(new Date(r.created_at).getTime() + 9 * 3600000)
      .toISOString()
      .slice(11, 19),
