...
export default function RestaurantPage({ restaurant: r, similar, govData }) {
  ...
  // 메타 desc
  const metaDesc = `${r.name} — 수지 ${r.type} 맛집. ${r.addr} 위치, 영업시간 ${formatHours(r.hours)}. Google 평점 ⭐${r.rt} (${r.cnt?.toLocaleString()}개 리뷰). ${r.tags?.slice(0,3).join('·')} 특징. 오늘뭐먹지 AI 추천.`
  ...
  return (
    <Layout title={`${r.name} | 수지 ${r.type}`} description={metaDesc} canonical={pageUrl}>
      ...
      <article style={{ maxWidth:760, margin:'0 auto', padding:'28px 16px 60px' }}>
        ...
        <CategorySuggestButton restaurant={r} region="suji" />
        ...
        <h2 style={h2style}>🍽️ 수지 {r.type} 맛집 더 보기</h2>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(200px, 1fr))', gap:10, marginBottom:28 }}>
              {similar.map((s, i) => (
                <Link href={`/suji/restaurant/${encodeURIComponent(s.name)}`} key={i}>
