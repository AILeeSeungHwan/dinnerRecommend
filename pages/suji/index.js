            ...
            <>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(240px, 1fr))', gap:10 }}>
                {matchedRestaurants.map((r, i) => (
                  <Link href={`/suji/restaurant/${encodeURIComponent(r.name)}`} key={r.name + i} style={{ textDecoration:'none', color:'inherit' }}>
                    <div style={{
...
function naverMapUrl(name) {
  const cleaned = name
    ...
    .replace(/ ([0-9]+호점)$/, '')
    .trim()
  return `https://map.naver.com/v5/search/${encodeURIComponent(cleaned + ' 수지')}`
}
      ...
      <div style={{ background:'var(--surface)',border:'1px solid var(--border)',borderRadius:20,padding:'28px 24px',maxWidth:380,width:'100%',textAlign:'center',boxShadow:'0 24px 64px rgba(0,0,0,.8)' }}
        ...
        {done && pickedR && (
          <div style={{ display:'flex',gap:8,justifyContent:'center',flexWrap:'wrap' }}>
            ...
            <a href={`https://map.naver.com/v5/search/${encodeURIComponent(pickedR.name + ' 수지')}`}
        ...
        {done && pickedRec?._external && (
          <div style={{ display:'flex',gap:8,justifyContent:'center',marginTop:8 }}>
            <a href={`https://map.naver.com/v5/search/${encodeURIComponent(pickedRec.restaurantName + ' 수지')}`}
              target="_blank" rel="noopener noreferrer"
...
const usageCnt = getUsageCount()
        ...
        (skipDbCheckRef.current && lastNoDataMenuRef.current
          ? `수지 맛집 추천. 사용자가 원래 "${lastNoDataMenuRef.current}"을 찾았으나 DB에 없어 연관 매장을 추천함. 각 reason에서 반드시 "${lastNoDataMenuRef.current}과 비슷한 이유" 또는 "${lastNoDataMenuRef.current} 대신 이 집을 추천하는 이유"를 자연스럽게 녹여줘. 단순 나열 금지.`
          : '수지 맛집 추천. 오늘 무엇을 원하는지 맥락을 파악해서 후보 중 딱 맞는 3곳 골라줘. 단순 나열 금지—상황과 감정에 맞게 큐레이션. reason에 "검색자"라는 단어 절대 금지.'),
          ...
          <div ref={resultsRef} style={{ marginTop:24, maxWidth:'100%', overflowX:'hidden' }}
              ...
              return pr ? (
                <div style={{ marginBottom:14,padding:'14px 16px',background:'rgba(255,107,53,.1)',border:'2px solid var(--primary)',borderRadius:12 }}>
                  ...
                  <div style={{ display:'flex',gap:6,flexWrap:'wrap' }}>
                    {prR && (
                      <Link href={`/suji/restaurant/${encodeURIComponent(prR.name)}`}
                    ...
                    {prR && <a href={`https://map.naver.com/v5/search/${encodeURIComponent(prR.name + ' 수지')}`}
                      target="_blank" rel="noopener noreferrer"
            ...
            {results[0]?._external && (
              <div style={{ marginBottom:14,padding:'10px 14px',background:'rgba(108,99,255,.07)',border:'1px solid rgba(108,99,255,.25)',borderRadius:10 }}>
                ...
                <div style={{ fontSize:'.72rem',color:'var(--muted)' }}>수지 커뮤니티·지도 앱에서 실시간 검색한 결과예요. 방문 전 영업 여부를 확인하세요.</div>
              </div>
            )}
            ...
            <div style={{
              ...
              {results.map((rec,i)=>{
                ...
                // ── 외부 검색 결과 카드 ──
                if (rec._external) {
                  const naverUrl = `https://map.naver.com/v5/search/${encodeURIComponent(rec.restaurantName + ' 수지')}`
                  // 3가지 형식 분기
                  const cardStyle = [
                    // 1번: 메뉴 중심 - 가격 뱃지 강조
                    ...
                    <a key={i} href={naverUrl} target="_blank" rel="noopener noreferrer"
                      ...
                      <div style={{ background:cardStyle.bg, border:`1px solid var(--border)`, borderLeft:`3px solid ${accentColor}`, borderRadius:14, padding:'16px 14px', cursor:'pointer', transition:'border-color .15s', height:'100%' }}
                        ...
                        <div style={{ display:'flex', gap:6, marginTop:8, alignItems:'center' }}>
                          <span style={{ fontSize:'.75rem', padding:'5px 12px', borderRadius:8, background:'var(--surface)', border:'1px solid var(--border)', color:'var(--muted)' }}>📍 {rec.address || '수지 근처'}</span>
              ...
              if (rec._notInDB && !r) {
                // DB에 없는 식당 → 네이버지도 카드
                return (
                  <a key={i} href={`https://map.naver.com/v5/search/${encodeURIComponent(rec.restaurantName + ' 수지')}`}
                    target="_blank" rel="noopener noreferrer"
...
export default function SamseongPage() {
  ...
  const [activeTab, setActiveTab] = useState(() => {
    if (typeof window !== 'undefined') return sessionStorage.getItem('suji-tab') || 'roulette'
    return 'roulette'
  })
  ...
  useEffect(() => {
    ...
    if (t && ['roulette','ai','browse','categories'].includes(t)) {
      setActiveTab(t)
      sessionStorage.setItem('suji-tab', t)
    }
  }, [router.query.tab])
  const topRated = [...restaurants].sort((a,b)=>b.rt-a.rt).slice(0,6)

  return (
    <Layout title="수지 맛집 추천 2026 | 수지구청역·풍덕천동·동천동 AI 가이드" description="수지 맛집 AI 추천 가이드. 수지구청역·풍덕천동·동천동·성복동 주변 국밥·이자카야·한우·중식·일식·양식 검증 식당을 날씨·기분·예산·카테고리에 맞춰 3초 만에 추천. 회식·데이트·혼밥·점심 상황별, 카카오맵·네이버 평점·리뷰·메뉴 가격 한눈에 비교." canonical="https://dinner.ambitstock.com/suji">
      <Head>
        <title>수지 맛집 추천 2026 | 수지구청역·풍덕천동·동천동 국밥·이자카야·한우·중식 AI 추천 | 오늘뭐먹지</title>
        <meta name="description" content="수지 맛집 AI 추천 가이드. 수지구청역·풍덕천동·동천동·성복동 주변 국밥·이자카야·한우·중식·일식·양식 검증 식당을 날씨·기분·예산·카테고리에 맞춰 3초 만에 추천. 회식·데이트·혼밥·점심 상황별, 카카오맵·네이버 평점·리뷰·메뉴 가격 한눈에 비교." />
        <link rel="canonical" href="https://dinner.ambitstock.com/suji" />
        <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
          "@context":"https://schema.org",
          "@graph": [
            {
              "@type":"ItemList","name":"수지 맛집 추천",
              "description":`수지구청역·풍덕천동·동천동 주변 ${restaurants.length}개+ 맛집 AI 추천.`,
              "url":"https://dinner.ambitstock.com/suji","numberOfItems":restaurants.length,
              "itemListElement":topRated.slice(0,10).map((r,i)=>({ "@type":"ListItem","position":i+1,"name":r.name,"url":`https://dinner.ambitstock.com/suji/restaurant/${encodeURIComponent(r.name)}`,"description":`${r.type} | ⭐${r.rt}` }))
            },
            {
              "@type":"BreadcrumbList",
              "itemListElement":[
      ...
      <section style={{ background:'linear-gradient(135deg, var(--surface) 0%, var(--bg) 100%)',padding:'32px 16px 24px',borderBottom:'1px solid var(--border)' }}>
        <div style={{ maxWidth:900,margin:'0 auto' }}>
          <div style={{ fontSize:'.75rem',color:'var(--muted)',marginBottom:8 }}>
            <Link href="/" style={{ color:'var(--muted)' }}>오늘뭐먹지</Link> › 수지
          </div>
          <h1 style={{ fontSize:'clamp(1.4rem,5vw,2.2rem)',fontWeight:900,marginBottom:8,lineHeight:1.2 }}>🏙️ 수지 맛집</h1>
          <p style={{ color:'var(--muted)',fontSize:'.88rem',marginBottom:14 }}>
            수지구청역·풍덕천동·동천동 <strong style={{ color:'var(--text)' }}>{restaurants.length}개+</strong> 식당 AI 추천
          </p>
          <div style={{ display:'flex',flexWrap:'wrap',gap:6 }}>
            {['#국밥','#한우','#이자카야','#중식','#회식','#가성비'].map(t=>(
      ...
      <div style={{ maxWidth:900,margin:'0 auto',padding:'20px 16px' }}>
              ...
              {CATS.map(cat=>{
                ...
                return (
                  <div key={cat.slug} style={{
                    ...
                    onMouseLeave={e=>{ e.currentTarget.style.borderColor='var(--border)'; e.currentTarget.style.boxShadow='none' }}
                  >
                    <Link href={`/suji/category/${cat.slug}`} style={{ display:'block', textDecoration:'none', color:'inherit', flex:1 }}>
              ...
              {posts.filter(p => p.region === 'suji').map(p => (
                <Link key={p.slug} href={`/posts/${p.slug}`} style={{ textDecoration:'none' }}>
                  <div style={{ display:'flex', flexDirection:'column', gap:6, background:'var(--surface)', border:'1px solid var(--border)', borderRadius:12, padding:8, cursor:'pointer', height:'100%' }}>
                    <PostThumbnail imageUrl={p.thumbnail} region="수지" category={p.category} slug={p.slug} size="100%" />
        ...
        <section style={{ marginTop:48,padding:'28px 20px',background:'var(--surface)',borderRadius:14,border:'1px solid var(--border)' }}>
          <h2 style={{ fontSize:'1.05rem',fontWeight:800,marginBottom:16 }}>수지구청역 맛집 가이드 — 지역별, 상황별 완전 정리</h2>
          <p style={{ color:'var(--muted)',fontSize:'.88rem',lineHeight:1.8,marginBottom:12 }}>
            수지구청역 맛집은 크게 세 구역으로 나뉩니다. 풍덕천동 일대는 직장인과 주민의 점심·퇴근 후 회식 수요가 집중되는 핵심 구역입니다. 수지구청역 주변 골목에는 빠른 점심을 해결할 수 있는 다양한 음식점이 모여 있으며, 현대그린푸드 인근 일대는 데이트·접대·기념일 식사에 어울리는 레스토랑이 자리합니다.
          </p>
          <p style={{ color:'var(--muted)',fontSize:'.88rem',lineHeight:1.8,marginBottom:12 }}>
            동천동 카페거리는 수지구청역 2번 출구 방향에 위치하며, 주말 브런치와 저녁 식사를 즐기는 커플·가족 방문객이 많습니다. 테이블 간격이 넓고 분위기가 차분한 레스토랑이 집중되어 있어 특별한 자리를 찾는 방문객에게 적합합니다. 수지구청역 상권 전반에 걸쳐 주차 공간이 넉넉한 편이어서 차량 방문자도 불편 없이 이동할 수 있습니다.
          </p>
          <p style={{ color:'var(--muted)',fontSize:'.88rem',lineHeight:1.8,marginBottom:12 }}>
          ...
          <p style={{ color:'var(--muted)',fontSize:'.88rem',lineHeight:1.8,marginBottom:12 }}>
            팀 회식 장소를 고르고 있다면, 수지구 인근 대형 고기구이 전문점과 이자카야가 가장 접근성이 좋습니다. 10인 이상 단체는 룸 예약이 가능한 곳을 2~3일 전에 확인하는 것이 안전합니다. 1차 고기구이 후 2차 이자카야로 이어지는 동선은 수지구 알파캠퍼스 서쪽 이면도로에서 대부분 해결 가능합니다.
          </p>
          <p style={{ color:'var(--muted)',fontSize:'.88rem',lineHeight:1.8,marginBottom:12 }}>
            접대나 파인다이닝을 원한다면 현대그린푸드·현대백화점 수지점 인근 예약제 오마카세와 코스 레스토랑을 우선 확인하세요. 1인 코스 기준 8만~20만 원 이상으로 범위가 넓으며, 예약은 최소 3~5일 전에 완료하는 것이 일반적입니다. 개인실(룸)과 주차 지원 여부를 예약 시 함께 확인하면 외부 손님을 모시기에 더 편리합니다.
          </p>
          <p style={{ color:'var(--muted)',fontSize:'.88rem',lineHeight:1.8,marginBottom:16 }}>
            수지구청역 맛집은 카테고리별로 세분화된 가이드를 제공합니다. 고기구이는 <Link href="/suji/category/meat" style={{ color:'var(--primary)' }}>수지구청역 고기구이·삼겹살</Link>, 회식 장소는 <Link href="/suji/category/group" style={{ color:'var(--primary)' }}>수지구청역 회식·단체</Link>, 데이트 레스토랑은 <Link href="/suji/category/date" style={{ color:'var(--primary)' }}>수지구청역 데이트·분위기</Link> 페이지에서 확인할 수 있습니다.
          </p>
          <p style={{ color:'var(--muted)',fontSize:'.88rem',lineHeight:1.8 }}>
            오늘 날씨·기분·예산을 입력하면 AI가 지금 상황에 딱 맞는 수지 맛집 3곳을 3초 만에 골라드립니다. 국밥부터 오마카세까지, 혼밥부터 30인 단체 회식까지 모두 커버합니다.
          </p>
        </section>
      </div>
    </Layout>
