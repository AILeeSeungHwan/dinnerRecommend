            ...
            <>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(240px, 1fr))', gap:10 }}>
                {matchedRestaurants.map((r, i) => (
                  <Link href={`/pangyo/restaurant/${encodeURIComponent(r.name)}`} key={r.name + i} style={{ textDecoration:'none', color:'inherit' }}>
                    <div style={{
...
function naverMapUrl(name) {
  const cleaned = name
    ...
    .replace(/ ([0-9]+호점)$/, '')
    .trim()
  return `https://map.naver.com/v5/search/${encodeURIComponent(cleaned + ' 판교')}`
}
      ...
      <div style={{ background:'var(--surface)',border:'1px solid var(--border)',borderRadius:20,padding:'28px 24px',maxWidth:380,width:'100%',textAlign:'center',boxShadow:'0 24px 64px rgba(0,0,0,.8)' }}
        ...
        {done && pickedR && (
          <div style={{ display:'flex',gap:8,justifyContent:'center',flexWrap:'wrap' }}>
            ...
            <a href={`https://map.naver.com/v5/search/${encodeURIComponent(pickedR.name + ' 판교')}`}
        ...
        {done && pickedRec?._external && (
          <div style={{ display:'flex',gap:8,justifyContent:'center',marginTop:8 }}>
            <a href={`https://map.naver.com/v5/search/${encodeURIComponent(pickedRec.restaurantName + ' 판교')}`}
              target="_blank" rel="noopener noreferrer"
...
const usageCnt = getUsageCount()
        ...
        (skipDbCheckRef.current && lastNoDataMenuRef.current
          ? `판교 맛집 추천. 사용자가 원래 "${lastNoDataMenuRef.current}"을 찾았으나 DB에 없어 연관 매장을 추천함. 각 reason에서 반드시 "${lastNoDataMenuRef.current}과 비슷한 이유" 또는 "${lastNoDataMenuRef.current} 대신 이 집을 추천하는 이유"를 자연스럽게 녹여줘. 단순 나열 금지.`
          : '판교 맛집 추천. 오늘 무엇을 원하는지 맥락을 파악해서 후보 중 딱 맞는 3곳 골라줘. 단순 나열 금지—상황과 감정에 맞게 큐레이션. reason에 "검색자"라는 단어 절대 금지.'),
          ...
          <div ref={resultsRef} style={{ marginTop:24, maxWidth:'100%', overflowX:'hidden' }}
              ...
              return pr ? (
                <div style={{ marginBottom:14,padding:'14px 16px',background:'rgba(255,107,53,.1)',border:'2px solid var(--primary)',borderRadius:12 }}>
                  ...
                  <div style={{ display:'flex',gap:6,flexWrap:'wrap' }}>
                    {prR && (
                      <Link href={`/pangyo/restaurant/${encodeURIComponent(prR.name)}`}
                    ...
                    {prR && <a href={`https://map.naver.com/v5/search/${encodeURIComponent(prR.name + ' 판교')}`}
                      target="_blank" rel="noopener noreferrer"
            ...
            {results[0]?._external && (
              <div style={{ marginBottom:14,padding:'10px 14px',background:'rgba(108,99,255,.07)',border:'1px solid rgba(108,99,255,.25)',borderRadius:10 }}>
                ...
                <div style={{ fontSize:'.72rem',color:'var(--muted)' }}>판교 커뮤니티·지도 앱에서 실시간 검색한 결과예요. 방문 전 영업 여부를 확인하세요.</div>
              </div>
            )}
            ...
            <div style={{
              ...
              {results.map((rec,i)=>{
                ...
                // ── 외부 검색 결과 카드 ──
                if (rec._external) {
                  const naverUrl = `https://map.naver.com/v5/search/${encodeURIComponent(rec.restaurantName + ' 판교')}`
                  // 3가지 형식 분기
                  const cardStyle = [
                    // 1번: 메뉴 중심 - 가격 뱃지 강조
                    ...
                    <a key={i} href={naverUrl} target="_blank" rel="noopener noreferrer"
                      ...
                      <div style={{ background:cardStyle.bg, border:`1px solid var(--border)`, borderLeft:`3px solid ${accentColor}`, borderRadius:14, padding:'16px 14px', cursor:'pointer', transition:'border-color .15s', height:'100%' }}
                        ...
                        <div style={{ display:'flex', gap:6, marginTop:8, alignItems:'center' }}>
                          <span style={{ fontSize:'.75rem', padding:'5px 12px', borderRadius:8, background:'var(--surface)', border:'1px solid var(--border)', color:'var(--muted)' }}>📍 {rec.address || '판교 근처'}</span>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(150px, 1fr))", gap:12 }}>
        {visible.map((r,i)=>(
          <React.Fragment key={i}>
            <SimilarRestaurantCard restaurant={{ ...r, reason: r.rt > 0 ? `평점 ${r.rt}점·리뷰 ${(r.cnt||0).toLocaleString()}건` : `리뷰 ${(r.cnt||0).toLocaleString()}건` }} regionPath="/pangyo" />
            {(i + 1) % 12 === 0 && (
...
export default function SamseongPage() {
  ...
  const [activeTab, setActiveTab] = useState(() => {
    if (typeof window !== 'undefined') return sessionStorage.getItem('pangyo-tab') || 'roulette'
    return 'roulette'
  })
  ...
  useEffect(() => {
    ...
    if (t && ['roulette','ai','browse','categories'].includes(t)) {
      setActiveTab(t)
      sessionStorage.setItem('pangyo-tab', t)
    }
  }, [router.query.tab])
    ...
    <Layout title="판교 맛집 AI 추천" description="판교·성남 주변 맛집 AI 추천. 국밥·이자카야·한우·중식 170개+ 식당." canonical="https://dinner.ambitstock.com/pangyo">
      <Head>
        ...
        <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
          ...
          "@graph": [
            {
              "@type":"ItemList","name":"판교 맛집 추천",
              "description":`판교역·테크노밸리·백현동 주변 ${restaurants.length}개+ 맛집 AI 추천.`,
              "url":"https://dinner.ambitstock.com/pangyo","numberOfItems":restaurants.length,
              "itemListElement":topRated.slice(0,10).map((r,i)=>({ "@type":"ListItem","position":i+1,"name":r.name,"url":`https://dinner.ambitstock.com/pangyo/restaurant/${encodeURIComponent(r.name)}`,"description":`${r.type} | ⭐${r.rt}` }))
            },
            {
              "@type":"BreadcrumbList",
              "itemListElement":[
                {"@type":"ListItem","position":1,"name":"오늘뭐먹지","item":"https://dinner.ambitstock.com"},
                {"@type":"ListItem","position":2,"name":"판교 맛집","item":"https://dinner.ambitstock.com/pangyo"}
              ]
            }
          ]
        })}} />
      ...
      <section style={{ background:'linear-gradient(135deg, var(--surface) 0%, var(--bg) 100%)',padding:'32px 16px 24px',borderBottom:'1px solid var(--border)' }}>
        <div style={{ maxWidth:900,margin:'0 auto' }}>
          <div style={{ fontSize:'.75rem',color:'var(--muted)',marginBottom:8 }}>
            <Link href="/" style={{ color:'var(--muted)' }}>오늘뭐먹지</Link> › 판교
          </div>
          <h1 style={{ fontSize:'clamp(1.4rem,5vw,2.2rem)',fontWeight:900,marginBottom:8,lineHeight:1.2 }}>🏙️ 판교 맛집</h1>
          <p style={{ color:'var(--muted)',fontSize:'.88rem',marginBottom:14 }}>
            판교테크노밸리·현대백화점·카카오 <strong style={{ color:'var(--text)' }}>{restaurants.length}개+</strong> 식당 AI 추천
          </p>
              ...
              {CATS.map(cat=>{
                ...
                return (
                  <div key={cat.slug} style={{
                    ...
                    onMouseLeave={e=>{ e.currentTarget.style.borderColor='var(--border)'; e.currentTarget.style.boxShadow='none' }}
                  >
                    <Link href={`/pangyo/category/${cat.slug}`} style={{ display:'block', textDecoration:'none', color:'inherit', flex:1 }}>
              ...
              {posts.filter(p => p.region === 'pangyo').map(p => (
                <Link key={p.slug} href={`/posts/${p.slug}`} style={{ textDecoration:'none' }}>
                  <div style={{ display:'flex', flexDirection:'column', gap:6, background:'var(--surface)', border:'1px solid var(--border)', borderRadius:12, padding:8, cursor:'pointer', height:'100%' }}>
                    <PostThumbnail imageUrl={p.thumbnail} region="판교" category={p.category} slug={p.slug} size="100%" />
