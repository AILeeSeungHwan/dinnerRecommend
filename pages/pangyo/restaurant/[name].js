...
export async function getStaticProps({ params }) {
  ...
  const govData = await fetchGovData('pangyo', r.name)
  ...
  if (cats.includes('이자카야') || cats.includes('야장') || cats.includes('와인바')) {
    if (isPremium || tags.some(t => t.includes('와인') || t.includes('위스키'))) {
      return { emoji:'🍷', lines:[
        ...
        `⭐${rt}점, ${cnt}개의 리뷰가 말해주는 판교 프리미엄 술자리.`,
      ]}
    }
...
export default function RestaurantPage({ restaurant: r, similar, govData }) {
  ...
  // 메타 desc
  const metaDesc = `${r.name} — 판교 ${r.type} 맛집. ${r.addr} 위치, 영업시간 ${formatHours(r.hours)}. Google 평점 ⭐${r.rt} (${r.cnt?.toLocaleString()}개 리뷰). ${r.tags?.slice(0,3).join('·')} 특징. 오늘뭐먹지 AI 추천.`
  ...
  const faqSchema = {
    ...
    "mainEntity": [
      ...
      { "@type":"Question", "name":`${r.name} 위치(주소)는?`, "acceptedAnswer":{ "@type":"Answer", "text":`경기 성남시 ${r.addr} (판교역 근처)` } },
      ...
      { "@type":"Question", "name":`${r.name} 주차 가능한가요?`, "acceptedAnswer":{ "@type":"Answer", "text": r.parking ? '주차 가능합니다. 상세한 주차 정보는 매장에 문의하세요.' : '판교역 인근 공영주차장을 이용하시거나 대중교통을 권장합니다.' } },
  ...
  const breadcrumbSchema = {
    ...
    "itemListElement": [
      ...
      slug && { "@type":"ListItem", "position":3, "name":`판교 ${catName}`, "item":`https://dinner.ambitstock.com/pangyo/category/${slug}` },
    ...
    <Layout title={`${r.name} | 판교 ${r.type}`} description={metaDesc} canonical={pageUrl}>
      ...
      <div style={{ background:'var(--surface)', borderBottom:'1px solid var(--border)', padding:'10px 16px' }}>
        <div style={{ maxWidth:760, margin:'0 auto', fontSize:'.75rem', color:'var(--muted)', display:'flex', gap:5, flexWrap:'wrap', alignItems:'center' }}>
          ...
          <Link href="/pangyo" style={{ color:'var(--muted)' }}>판교 맛집</Link> <span>›</span>
          {slug && <><Link href={`/pangyo/category/${slug}`} style={{ color:'var(--muted)' }}>판교 {catName}</Link> <span>›</span></>}
      ...
      <article style={{ maxWidth:760, margin:'0 auto', padding:'28px 16px 60px' }}>
        ...
        {/* CTA 배너 — 지역 AI 추천 유도 */}
        <a href="/pangyo?tab=roulette" style={{ textDecoration:'none', display:'block', marginBottom:20 }}>
          <div style={{
            ...
            <div style={{ flex:1 }}>
              <div style={{ color:'#fff', fontWeight:900, fontSize:'1rem', marginBottom:4, lineHeight:1.3 }}>
                판교역 뭐 먹을지 아직 못 정하셨나요?
              </div>
              <div style={{ color:'rgba(255,255,255,.85)', fontSize:'.8rem', marginBottom:10 }}>
                판교테크노밸리·현대백화점 맛집 AI 추천 — 날씨·기분·예산에 맞게 AI가 바로 골라드려요
              </div>
              <span style={{
        ...
        <table style={{ width:'100%', borderCollapse:'collapse', fontSize:'.88rem', marginBottom:28 }}>
          <tbody>
            {[
              ...
              ['판교 4번출구', r.exit4 ? '✅ 도보 3분 이내' : '판교 도보권 내'],
            ].map(([label, val], i) => (
        ...
        <OperatorNote restaurant={r} regionName="판교" totalRegionRestaurants={480} govData={govData} />
        ...
        {matchedWx.length > 0 && (
          <>
        <PostMidCategoryBanner region="pangyo" regionName="판교" restaurant={r} />
        ...
        <ul style={ulstyle}>
          <li style={listyle}><strong>지하철</strong> — 2호선·수인분당선 판교 하차</li>
          {r.exit4
            ? <li style={listyle}><strong>도보</strong> — 4번출구 기준 약 3분 이내</li>
            : <li style={listyle}><strong>도보</strong> — 판교 각 출구에서 도보 5~10분 내외</li>}
        ...
        {similar?.length > 0 && (
          <>
        <h2 style={h2style}>🍽️ 판교 {(r.cat && r.cat[0]) || r.type} 맛집 더 보기</h2>
            <p style={pstyle}>
              <strong>{r.name}</strong>와 같은 카테고리에서 평점·가격·거리 기준으로 한 곳씩 골라봤습니다.
            </p>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(180px, 1fr))', gap:12, marginBottom:28 }}>
              {similar.map((s, i) => (
                <SimilarRestaurantCard key={i} restaurant={s} regionPath="/pangyo" />
              ))}
            </div>
          </>
        )}
        ...
        <div style={{ display:'flex', gap:10, flexWrap:'wrap', paddingTop:20, borderTop:'1px solid var(--border)' }}>
          {slug && (
            <Link href={`/pangyo/category/${slug}`}
