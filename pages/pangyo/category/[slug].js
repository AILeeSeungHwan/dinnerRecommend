  ...
  return (
    <>
      ...
      <Head>
        ...
        <meta name="description" content={`판교역·판교테크노밸리·현대백화점 판교점·백현동·삼평동 주변 ${catInfo.name} 맛집 ${restaurants.length}곳을 카카오맵·네이버 평점·리뷰·메뉴 가격 기준으로 정리. ${catInfo.keywords}. 회식·데이트·혼밥·점심 상황별 추천, 영업시간·주차·예약 정보까지 한눈에.`} />
        ...
        <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
          ...
          "@graph": [
            { "@type": "ItemList", "name": `판교역 ${catInfo.name} 맛집`, "description": `판교역 ${catInfo.name} 맛집 ${restaurants.length}곳 추천.`, "url": `https://dinner.ambitstock.com/pangyo/category/${slug}`, "numberOfItems": restaurants.length,
              "itemListElement": sorted.slice(0, 10).map((r, i) => ({ "@type": "ListItem", "position": i + 1, "url": `https://dinner.ambitstock.com/pangyo/restaurant/${encodeURIComponent(r.name)}`, "item": { "@type": "Restaurant", "name": r.name, "address": r.addr, ...(r.rt ? { "aggregateRating": { "@type": "AggregateRating", "ratingValue": String(r.rt), "reviewCount": String(r.cnt || 0) }} : {}) } }))
            },
            { "@type": "BreadcrumbList", "itemListElement": [
              ...
              {"@type":"ListItem","position":3,"name":`판교 ${catInfo.name}`,"item":`https://dinner.ambitstock.com/pangyo/category/${slug}`}
            ]}
          ]
        })}} />
      </Head>
      ...
      <div className="container" style={{ padding:'24px 16px' }}>
        ...
        <div className="thumb-grid">
          {sorted.slice(0, visibleCount).map((r, i) => (
            <SimilarRestaurantCard key={i} restaurant={{ ...r, reason: r.rt > 0 ? `평점 ${r.rt}점·리뷰 ${(r.cnt||0).toLocaleString()}건` : `리뷰 ${(r.cnt||0).toLocaleString()}건` }} regionPath="/pangyo" />
          ))}
        </div>
